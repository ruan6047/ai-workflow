...
.. warning::
   ...
   **接線之後，下列四件仍然不成立，寫報告與卡面時不得含混：**
   ...
   1. **``allow`` 不是「歸屬已被驗證」，只是「這筆登記的宣告與卡相符」。**
      ...
      **這是 danger 第 1 條在判定層的倒影，不是新問題。** 今天的 assign 發生在建立
      之前，而「歸屬」這個事實在建立之後才存在——所以這一刻的單點檢查拿不到它。

      ⚠️ **但這個先後是操作慣例，不是結構必然；本檔前一版把它寫成了後者。**
      ``templates/worktree-lifecycle.md`` 第 1 點規定的順序其實**相反**：claim 成功後
      **先建立 worktree**，再把「**實際路徑**＋分支」寫回卡面。是今天的 ``assign`` 把
      claim 與登記併成同一步，才使登記必然早於建立——下面第 4 條稱它「生產常態」，
      那是**實況的描述，不是定律**。
      **實測（2026-08-16，真實磁碟、非 fixture）**：把順序倒過來（登記時目標已存在），
      §8.3 那兩個至今仍在磁碟上的跨 repo 漂移 worktree 會被擋下——軸 A 仍然放行
      （沒有明示宣告，見第 2 條），擋下它們的是**軸 B**：``contradiction``／``refuse``，
      ``assign`` 回 6。
      要不要讓 ``assign`` 要求目標先存在，是**需求方的取捨**——代價是 assign 從此只能
      在 worktree 所在的那台機器上執行（軸 B 是機器局部的，見第 4 條）。
      **本卡未做，本段不得被讀成已做。**
