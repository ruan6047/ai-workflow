...
⚠️ 而**守衛的形狀**又是第三輪被擋下的原因：R2 用結構斷言釘住「（」與第一個事實
之間不得插字，並宣稱「任何新標籤必然插在該位置，因此必然被抓」。查核者把同一個
標籤插到第一個事實**之後**，測試全綠（R2-001 blocking）。教訓寫在這裡以免第四代：
**措辭是開放集合，位置也是**；用「不可以是什麼」去守一個開放集合，永遠會有下一個
沒被列舉到的成員。現行守衛因此改為封閉形狀——見 ``AUTHORITY_NOTE_TEMPLATE``。
...
四輪的教訓正是不要再對開放集合下全稱宣稱。這裡只說這四個跑過、都紅。

M27 **記為已知不涵蓋，不修**。理由不是它不重要，是它不屬本守衛要擋的形狀：AST
釘的是 ``return`` 的**語法**，不約束 ``author`` 這個**值**的來源；要關掉它就得約束
資料流，而關掉資料流之後還有裝飾器、``_resolve_ruling_author`` 內部、以及執行期
monkeypatch。**對擁有這份碼的人，任何測試與任何執行期檢查都無效。**
...
4. 模板與測試黃金值兩邊同時改錯仍會綠。守衛保證「被看見」，不保證「被看對」。
...
#: 這個**值**怎麼來——在 ``return`` 之前改寫 ``author`` 即可注入標籤（M27／R4-001），
#: 該形狀**已知不涵蓋且不修**。完整理由、四條已知的縫、以及需求方那句「不能證明
#: M27 不會發生」的原文，見模組 docstring「授權註記守衛的威脅模型」。
