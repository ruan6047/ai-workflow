"""消費 core/verbs.md §1 review 列（本機分支頭比對、git 附錄取源與失敗逐字）、
core/return.md「卡與身分」列。

本機 git 的唯讀 revision 入口，子指令集合恰三個：`rev-parse`、`log`、`diff --stat`。
不連網、不寫本機 repo、不改工作樹、不解析輸出語意（第零條）；`gh/localgit.py` 的
merge-tree 不變式與本檔無關，兩者各自只包自己那組子指令。
取不到就 raise，交呼叫端印一行帶原因：⛔ 不回空清單冒充「沒有改動」，也⛔ 不推論本機與遠端相同。
"""
...
def rev_parse(revision, *, root='.', runner=None):
    """解到 40 碼 commit SHA。本機沒有這個 revision＝None（事實缺席，⛔ 不是錯誤：`--verify --quiet`
    此時 rc≠0 而 stderr 空）；root 非工作樹或 git 不可執行＝LocalRevUnavailable（stderr 逐字）。"""
...
def log_commits(base, head, *, root='.', runner=None):
    """`git log --no-color --pretty=format:%H %s <base>..<head>` 的每列一筆（兩點＝只列 head 這一側
    新增的 commit）。真的沒有新 commit＝空 list；失敗一律 raise，⛔ 不與空 list 混用。"""
    return _lines(_run(root, ('log', '--no-color', LOG_FORMAT, f'{base}..{head}'), runner), 'git log')


def diff_stat(base, head, *, root='.', runner=None):
    """`git diff --stat --no-color <base>...<head>` 的每列一筆（三點＝左端取 merge-base）。
