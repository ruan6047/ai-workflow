...
| 四版 | 候選路由行用 `startswith("- 執行：")` 嚴格收集 | 第二條路由行前置一個 U+200B 就從候選集消失，`ambiguous` 被降成 `matched`——**唯一一次錯在放行側**（見下節） |

前三版是同一個病的三層：**把「出現」當成「宣告」**（先是內容出現，後是字串出現）。
宣告不是出現，**宣告是結構位置**。第四版的病不同——不是認錯宣告，而是**掃描時漏看**，
且漏看的方向會鬆綁義務。
...
#### 正規化只加在偵測側——判準是**錯誤的方向**，不是「有沒有結構層」

先前這一節寫「標記機制已在結構層解決，所以不需要正規化」。**那個論證的適用邊界劃錯了**
（R4-003）：它對**標記辨識**成立，對**多行候選掃描**不成立——掃描本身就是內容比對，
零寬字元照樣能讓一行「消失」。
...
R4-003 就是後者：候選集原本用 `startswith("- 執行：")` 收，第二條路由行前置一個 U+200B
就整條被略過，於是「兩條路由行 → `ambiguous`」被降成「剩一條 → `matched`」。這是本能力
歷來唯一一次錯在放行側——前幾輪都錯在多要求理由（保守側）。

所以正規化**只加在偵測側**，且只用來**擴大**候選集：

- **偵測（誰算候選）**：寬鬆。NFKC 折疊全形半形，去掉 Unicode 類別 `Cc`／`Cf`／`Mn`／
  `Me` 的不可見與格式字元，去掉所有空白，再比前綴。**寧可多收**——多收只會讓候選變多、
  更容易 `ambiguous`。
- **受理（候選能否當基線）**：嚴格。一律用**原始行**比對，不套用任何偵測正規化。不合格
  → `ambiguous`。

兩側都不可能產出偽 `matched`：多收只會更嚴，受理端從不放寬。**偵測的寬鬆只決定「這行
要不要被檢查」，絕不用來幫破損的卡面補正**——後者才是「重新造出一個猜測層」。

> **誠實邊界**：偵測正規化涵蓋不可見／格式字元、空白與全形半形，**不宣稱能窮盡所有
> Unicode 擾動**。保證的是**方向**（漏收才危險，而漏收的面已被壓到最小）而非完備；真有
> 漏網的擾動，仍受「宣告必須緊鄰唯一候選」這條純結構規則保護。測試除了逐案例列舉，另有
> 一條**性質斷言**——在 Log 之前每一行每個位置插入每種不可見字元，斷言「原本要理由的
> 卡面破損後仍要理由」，因為列舉永遠會漏下一種擾動。
...
| 標頭區有第二條路由行，且其前綴被零寬字元／全形半形差異遮蔽 | `ambiguous`（候選寬鬆收集，遮蔽不了） |
