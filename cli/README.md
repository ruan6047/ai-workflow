...
|---|---|---|---|
| `matched` | 卡面有建議且四欄全合格，建議＝實際 | 否 | `（與卡面建議 X 相符）` |
| `deviated` | 卡面有建議且四欄全合格，建議≠實際 | **是** | `（偏離卡面建議 X；偏離理由：…）` |
| `absent` | 卡面**完全沒有新制痕跡**：無「- 執行：」行，或該行是規劃期路由必填之前的舊格式（#17／#19／#20／#21 那批） | **是** | `（卡面無建議層級：…；理由：…）` |
| `ambiguous` | 卡面**看得出有建議但讀不出可信的層級**：行不唯一、body 排版損壞，或任一欄不合格 | **是** | `（卡面建議無法解析：…；理由：…）` |

#### `absent` 與 `ambiguous` 的分界（R2-001 修正）

這條界線一度畫錯：原本「正規表示式沒匹配」就一律當成舊卡（`absent`），於是把
`）　查核：` 的全形空白改成半形，卡面明明寫著建議，Log 卻寫「卡面無建議層級」——不實
留痕，正是本節其他規則要防的事。同一個洞還讓「理由被清空」的卡判成 `matched`，反而
**免除**了理由要求。

現在分兩段判：先看該行是否**自稱**新制（含「建議」字樣或任一能力層級值），自稱了就
必須完整合格，否則 `ambiguous`；完全沒有痕跡才是 `absent`。誤判方向刻意偏向
`ambiguous`——多打一個旗標的成本，遠低於寫下不實留痕。
...
上表每一列都有對應測試，且各有一條「Log 不得寫成『卡面無建議層級』」的斷言。
...
2. **依賴 body 沒被手改壞**：因此**不猜**——定位不到、不唯一、任一欄不合格，一律
   歸 `ambiguous` 要求理由，而不是「當作沒有建議」悄悄放行（分界與破壞方式列舉見上）。
