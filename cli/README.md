...
| `absent` | **無版本標記宣告**＝規劃期路由必填之前開的卡（#7–#25 全部屬此，且**永久**如此，見下） | **是** | `（卡面無建議層級：…；理由：…）` |
| `ambiguous` | **有版本標記但讀不出可信的建議**：合格路由行不是恰一行，或任一欄不合格；body 排版損壞亦然 | **是** | `（卡面建議無法解析：…；理由：…）` |

#### `absent` 與 `ambiguous` 的分界＝版本標記 `<!-- wf-routing:v1 -->`（R3-001／R4-001）
...
| 三版 | 標記**字串有沒有出現**（`ROUTING_MARKER in head`） | `amend` 能把任意文字寫進 Log 之前——舊卡的驗收條件只要提到這串標記，就從 `absent` 誤升 `ambiguous`。入口不在使用者手打，在本 CLI 自己的 `amend` |
| 四版 | 候選路由行用 `startswith("- 執行：")` 嚴格收集 | 第二條路由行前置一個 U+200B 就從候選集消失，`ambiguous` 被降成 `matched`——**錯在放行側**（見下節） |
| 五版 | 候選以「NFKC ＋ 剝除 `Cc`／`Cf`／`Mn`／`Me` 與空白」後比前綴 | 前置 U+02B0（`Lm`）或 U+0378（`Cn`）等**不在剝除清單裡**的碼位，第二條路由行照樣消失 → 同一個放行側失敗（見下節） |

前三版是同一個病的三層：**把「出現」當成「宣告」**（先是內容出現，後是字串出現）。
宣告不是出現，**宣告是結構位置**。四、五版的病不同也相同——不是認錯宣告，而是**掃描時
漏看**，且漏看的方向會鬆綁義務；四版靠字面前綴、五版靠字元類別清單，兩者都是在**列舉
「什麼算雜訊」**。列舉一個開放集合必然還有下一次，所以現行判準改成列舉「什麼算已知」，
其餘一律進候選（見下節）。

**現行機制**：`open` 在新制卡的路由行上方寫入 `<!-- wf-routing:v1 -->`。分類承認它是
宣告，需要**三個結構條件同時成立**：

1. **獨立成行**——整行 `strip` 後恰等於標記；行內夾帶一律不算。
2. **位於標頭區**——第一個 `## ` 標題之前。`amend` 的驗收／驗證／資源宣告都寫在
   `## ` 章節內，結構上碰不到標頭區。
3. **緊鄰唯一路由行**——標記必須就在它所宣告的那一行上方。

三者任一不成立就沒有宣告 → `absent`；宣告成立卻讀不出合格路由行 → `ambiguous`。
標頭區內唯一其他可寫入路徑 `amend --spec-baseline` 已擋掉換行（單行欄位必須保持單行），
避免它長出偽宣告行。

標記沿用本 repo 既有的 HTML 註解慣例（`resources.py` 的 `<!-- resource-claims:begin -->`、
`doctor.py` 的 `<!-- wf-review-event:v1 ... -->`），不另創一套；與 doctor 的事件 marker
不碰撞（後者只掃 Issue **留言**與 PR review body 且鎖 `wf-review-event:` 前綴）。標記只
認**Log 之前**的區段，Log 引用到它不算數。

#### 既有卡永久以 `absent` 派工（需求方裁定，非過渡安排）

本 CLI **沒有、也不會有**補寫 `wf-routing:v1` 標記的入口。開卡於本能力之前的既有卡
（#7–#25 全部）**永久**落 `absent`：每次 `assign` 都要帶 `--capability-deviation-reason`，
直到該卡結案為止。這是需求方 2026-08-11 的裁定，理由有三：

1. **能寫入標頭區的入口就是漏洞來源**——上面那張表的第三列（R4-001）正是這麼被打穿的。
   新增一個「受控補標記」的入口，就是新增第二個必須自證不會變成同一個洞的入口。
2. **事後補填的四個路由值未必反映規劃期判斷**，違反 `MODEL_ROUTING.md`「路由決定於
   規劃期」——補出來的建議是回溯編造的基線，比沒有基線更糟。
3. 代價（既有卡每次派工多帶一個理由，且該理由實質是雜訊）**需求方已知悉並接受**。

所以請不要把既有卡的 `absent` 當成「等遷移工具做好就會消失」的暫態——它就是這些卡的
最終行為。新卡自 `open` 起一律帶標記，不受影響。

#### 候選路由行＝「已知非路由行」的**補集**——不列舉字元類別（R4-003／R5-001）

判準要看**失敗會往哪邊倒**：

| 步驟 | 比對失敗的後果 | 方向 |
|---|---|---|
| 標記辨識（是否有宣告） | 少一個宣告 → `absent` → **要求理由** | 保守，安全 |
| 候選路由行收集 | 少一條候選 → 可能剛好剩 1 條 → `matched` → **免除理由** | **危險** |

這一段錯在下面那一列**兩次**，形狀相同：

| 版本 | 候選判準 | 被什麼打穿 |
|---|---|---|
| 四版（R4-003） | `startswith("- 執行：")` | 第二條路由行前置 U+200B 就掉出候選集 |
| 五版（R5-001） | NFKC 折疊 ＋ 剝除 `Cc`／`Cf`／`Mn`／`Me` 與空白後比前綴 | 前置 U+02B0（`Lm`）或 U+0378（`Cn`）——**不在剝除清單裡**的碼位，照樣掉出候選集 |

兩次的修法都是「再多列舉一類字元」。Unicode 的碼位與類別是開放集合，**列舉必然還有下
一次**。真正的病不是清單不夠長，是**未知輸入被推到危險的那一側**：沒被列舉到的字元讓
一行從候選集消失，剩下的唯一候選剛好緊鄰標記，於是 `matched` 並免除偏離理由。

**現行判準把承擔未知的那一側翻過來**：

> 標頭區的一行，只有在它被**正面辨識**為 `render_issue_body` 會產出的某一種已知非路由行
> （`- 需求：`／`- Initiative：`／`- DB：`／`- 服務的原始目標：`／版本標記／純空白行）時
> 才**不算**候選。**其餘一律是候選，包括任何系統看不懂的行。**

由此得到一條可機械檢查的單調性：**往標頭區插入任何字元或任何一整行，只能讓那一行從
「已知」掉進「候選」，不能反向**——因為「已知」的判準是**原始行**對固定字面前綴的
`startswith`，插入字元只會破壞前綴比對（→ 變成候選 → 更嚴），不可能憑空造出前綴。
未知碼位因此天然落在保守側，程式碼裡不再有任何「哪些字元算雜訊」的清單。

受理側（候選能否當基線）維持嚴格：一律用**原始行**比對，不套任何正規化。**偵測的寬鬆
只決定「這行要不要被檢查」，絕不用來幫破損的卡面補正**——後者才是「重新造出一個猜測層」。

> **這裡宣稱什麼、不宣稱什麼**（不寫「已窮盡」）：
>
> - **有機械證明**：`test_no_single_codepoint_prefix_can_remove_a_line_from_the_candidate_set`
>   掃 U+0000–U+10FFFF 全部 1,112,062 個碼位（扣代理區與換行），每一個當前綴都不得讓那行
>   離開候選集；`test_candidate_set_is_monotone_under_arbitrary_line_insertion` 證明插入
>   整行只會讓候選變多；兩條逐位置性質測試斷言掃描覆蓋的 (行號, 位置, 字元) 三元組**等於**
>   獨立算出的全集，而不是「樣本數大於某個值」。
> - **不宣稱**：把路由行的內容**藏在某個已知前綴後面**（例如寫成 `- DB：… - 執行：…`）時
>   一定會被算成候選。這一面由一條**非承載性**的額外收緊處理（已知前綴行若仍帶有
>   `執行：` 形狀就取消豁免，同一已知前綴出現兩次亦取消豁免）；它的漏網只會退回上面那條
>   補集保證，不會低於它。**這個洞沒有關掉，見下方「仍未關掉的面」。**

#### 仍未關掉的面（明列，不假裝沒有）

1. **借殼行**：有人把路由行接在已知前綴後面，且把 `執行：` 寫成收緊判準認不出的形狀，
   該行就享有豁免、不進候選集。此時卡面仍有一條**緊鄰標記的合格路由行**被當成基線——
   系統讀到的是標記所宣告的那一行，不是借殼那一行。判斷是：標記寫在開卡當下、位置由
   渲染端決定，用它當權威比用「哪一行看起來像」當權威更可靠；但**這不等於借殼行不存在**。
2. **同一行內的語意攻擊**：合格路由行本身被改成另一組合法值（例如把建議層級從「高階型」
   改成「經濟型」），任何結構判準都看不出來——那是 body 被有權限者竄改，屬 GitHub 端的
   權限問題，不是解析器能承擔的。
3. **收緊判準的誤判**：某張卡的 `spec 基線`／`服務的原始目標` 若真的引用了「執行：」字樣，
   該卡會落 `ambiguous`，派工時多帶一個理由。保守側的雜訊，不是放行。

層級值裡混入零寬字元同樣落 `ambiguous`（受理端 `strip` 後查不到表），這是正確的
fail-closed 結果。
...
| 標記出現在行內而非獨立成行 | `absent`（不成立為宣告） |
| 標記出現在 `## ` 章節內（含 `amend` 寫入的驗收／驗證文字） | `absent`（不成立為宣告） |
| 標記在標頭區但未緊鄰路由行 | `ambiguous` |
| 標頭區有兩個以上標記 | `ambiguous` |
| 路由行前綴混入零寬／格式字元（U+200B、U+FE0F） | `ambiguous` |
| 標頭區有第二條路由行，其前綴被**任何單一碼位**（零寬字元、`Lm`、`Cn`、全形半形差異…）遮蔽 | `ambiguous`（候選＝已知非路由行的補集，遮蔽不了；全碼位掃描為證） |
| 標頭區被插入一整行任意內容 | `ambiguous`（多一個候選） |
| 已知欄位行（如 `- DB：`）後面接上路由行形狀 | `ambiguous`（取消該行豁免；**非承載性**收緊，見上方「仍未關掉的面」） |
| 同一個已知欄位前綴在標頭區出現兩次 | `ambiguous`（重複者全數回到候選集） |
...
上表每一列都有對應測試。**路由行本身被破壞**那一組（`tests/test_card.py` 的
`CORRUPTED_ROUTING_LINES`，即本表前七列）另有一條「Log 不得寫成『卡面無建議層級』」斷言；
舊卡類的每一列另有一條「Log 必須寫『卡面無建議層級』而非『偏離』」斷言。標頭區結構類的
幾列斷言的是 `outcome` 與 `requires_reason`，不另外斷言 Log 措辭。
