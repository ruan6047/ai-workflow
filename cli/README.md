...
| `amend` | 開卡後修訂卡面：spec 基線／驗收條件／驗證項目／資源宣告／`級別`；`--reason` 必填，每個被改欄位各 append 一行 Log 記下**完整原值**（不截斷，Log 是唯一還原點）並帶同一 `op` 識別碼；值未變、內容為空、錨點不唯一一律拒絕（不寫不實留痕）；清單替換預設重設未勾選，`--preserve-checked` 才沿用；`級別` 先寫並讀回驗證再寫 body，欄位寫失敗即乾淨中止；修訂只作用於 `## Log` 之前，排版壞掉時拒絕，另由 `--repair-log-layout`＋`--expect-body-sha256` 提供只准動空白的窄路修復；`--record-unlogged-change` 補救「欄位已改但 Log 沒記」的半寫入；`--dry-run` 零遠端寫入 | 寫（有條件拒絕） |
...
一切修訂以免誤動 Log。此旗標是唯一出路，且刻意窄到只認**一個精確的事故形態**：

- 必須同時給 `--expect-body-sha256`，且不得與其他修訂旗標併用。
- **只把首個字面 `\n## Log\n\n` 換成真正的標題與空行，其餘每一個字元逐位元不動。**
  出現 0 次或 2 次以上、或該標記落在 fenced code block 內，一律拒絕。
- 修復後必須能安全定位 Log，且原本可解析的資源宣告須仍解析成相同結果。

這個「不談範圍、只做一次定點替換」的設計是兩輪查核打出來的。先前兩版都敗在同一個
錯誤——以為限縮了範圍，實際仍在做無差別替換：

- 第一版對整份 body 替換所有字面 `\n`，並以「剝掉空白後相同」當內容不變的證明。
  該證明預先把字面 `\n` 從比較基準刪掉，等於把破壞藏進證明裡；JSON 字串裡的
  `{"note": "a\nb"}` 因此被改壞而檢查不出來。
- 第二版限縮了起點卻仍對整個尾段無差別替換。實測兩個破口：#17 自己的 Log 敘述含
  合法的字面 `\n`（那行正在描述「把字面 `\n` 還原」）會被一併改掉；fenced code
  block 內的 `\n## Log` 會被誤認為 Log 起點。

若遇到本形態以外的排版損壞，`amend` 會拒絕而不猜。那種情況請人工處理，並考慮它是
否代表某處仍在繞過 `wfcli` 直接寫 body。
