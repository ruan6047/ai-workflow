...
- 產出＝一張卡：卡面判準欄（核心痛點、非射程、服務的原始目標、`when`）、階段計畫、級別與三軸、能力層級、`db_scope`、資源宣告、清單收斂宣告全部填實。
- 交回物是卡面文字，⛔ 不是成品。
...
- 離開＝`open` 印的缺欄清單為空、PM 判 R1 R2 過、需求方未否決；T2 以上下一階段必含規劃，缺規劃由 `move` 印。
...
- 高複雜或影響大的卡把研究列進階段計畫；交付物是排程、爬蟲、告警等外部觸發時列維護。
- 純文件卡⛔ 不列部署。

## 5 · 各角色做／⛔ 不做

- 需求方：決定升級、判痛點還成立嗎；⛔ 不代填表單。
...
- F-需求-02：只寫可觀測現象（「X 指向 Y，但 Y 不存在」），⛔ 不寫解法、⛔ 不寫未量測的因果推論。
- F-需求-03：開卡前讀清單項全部留言，⛔ 不只讀 body。
- F-需求-04：開卡前三問：服務哪個目標、執行者是誰、現在有人受害嗎。
- F-需求-05：一根問題一張卡；開新卡僅限需要不同能力域的執行者、紅線隔離、寫入集不相交可真平行三情形。
- F-需求-06：鏈深沿父鏈算，>2 只印；全域問題脫鏈獨立運行，⛔ 不入鏈。
- F-需求-07：待審清單不佔投影欄、不進盤點分母；清單當墓地可接受。
