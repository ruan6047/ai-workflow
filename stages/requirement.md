...
- 交出一張卡：填實核心痛點、非射程、服務的原始目標、`feature`、`when`、階段計畫、級別與三軸、能力層級、`db_scope`、資源宣告、清單收斂宣告。
- 只交卡面文字，⛔ 不交成品。
- 缺陷走同一階段、同一卡面：把重現步驟、預期與實際、根因、回歸測試寫進核心痛點與驗收條件。

## 2 · 進入／離開條件

- 從需求方點頭升級的清單項經 `open` 上板進入（`core/verbs.md`）；缺陷開不開卡由需求方決定，不開卡時直接 commit。
- `move` 印的建卡必填缺欄為空、PM 判 R1 R2 過、需求方未否決才離開；T2 以上把規劃列進下一階段，缺規劃由 `move` 印。
- 撤銷用 `move --to 清單`，保留卡ID、延續 iteration；判「現在不做」時走撤銷，⛔ 不走停止。
- 注意事項候選要升為 P- 或 F- 條目時，提案填三格：條文、來源（留言 URL）、處理手段；缺一格⛔ 不提。
...
- 級別取三軸各自定級後的最高（`core/tiers.md` §2）；級別依據三格填在 `tier_basis`。
