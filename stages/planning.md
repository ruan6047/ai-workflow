...
- ① `notes --stage 規劃` ② PM `brief --for executor` 派寫規格 ③ 執行者起草規格欄、`review --file --role executor` 交回；需求方核可後由 PM 以 `edit --ruling` 落欄（`roles/pm.md` §3、`core/conflict-resolution.md` §1） ④ PM 有序判：R1 上游產出還有效嗎 → R2 射程對核心痛點 → 每條驗收條件可追溯回痛點、非零資訊、基線釘死 ⑤ `move`。
- 規格欄的變更經需求方裁定才生效，落卡面的操作依 `roles/pm.md` §3。
- 父卡基線變更時先更新父卡、標註受影響的帶 `parent` 的卡並重新核可，再繼續；⛔ 不只在下層卡內改方向。

## 5 · 各角色做／⛔ 不做

- 執行者：起草規格、驗收條件、驗證項目；⛔ 不落欄、⛔ 不改核心痛點、⛔ 不在未回寫下改已核可方向。
- PM：判 R1 R2 與驗收條件可追溯、依核可裁定逐字落欄；⛔ 不判技術取捨、⛔ 不改寫執行者起草的內容。
- 需求方：核可取捨與驗收條件；T4 質詢；⛔ 不寫規格。
- 查核者：不在本階段。
- `.github/scripts/source_budget.py` 的行數治理軟警示觸發後，規劃者須在交回單寫出責任邊界與行數增長原因；⛔ 不以壓行代替說明。
...
- F-規劃-07：核心痛點的成功條件與裁定矛盾時，由執行者提更正案、需求方裁定、PM 以 `edit --ruling` 逐字回寫並列排除與歸屬；⛔ 不縮射程、⛔ 不由當階段執行者逕改。
