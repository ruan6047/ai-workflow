...
- 事後查核發現缺陷時，是否回退 main 交需求方裁定；修復卡命名依 `core/naming.md` §1。

## 5 · 各角色做／⛔ 不做

- PM：merge、收尾、組裁定單、跑 `move` 進終態（關 issue 與封存是該動詞的副作用、由 CLI 在進終態時完成，`core/verbs.md` §1 `move` 寫欄；⛔ 不是 PM 另做的人工動作，`core/conflict-resolution.md` §1）；⛔ 不裁定停止。
