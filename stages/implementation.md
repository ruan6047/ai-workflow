...
- 失聯：卡停在進行中；PM `move` 到待確認再退回、再派（iteration +1），或另一實體以 `role=executor` 交回，`mistakes` 每則 `what` 起首逐字「非本人自評」；分支未 push 則走阻塞（`wf-ruling` kind=block）。
...
- F-執行-04：artifact 在交付 SHA 可重現：產生工具與 artifact 同一 commit；自指命中明列，⛔ 不偷偷排除。
- F-執行-05：資源宣告漏列交付檔、宣告過寬、指到不存在路徑，交回前對照 `git diff --name-status` 修正。
