...
- 派工單本身在被審範圍，`attribution` 歸屬依 `roles/reviewer.md` §1。
...
- PM：派審、判裁決完整性；⛔ 不判裁決對錯、⛔ 不改寫裁決內容（逐字代貼例外＝`roles/reviewer.md` §3，首行標記依 `core/naming.md` §3）。
- 需求方：T4 sign-off；升級四選一裁定。
- 執行者：收退回後修；⛔ 不自審（T2 以上）。
- `.github/scripts/source_budget.py` 的行數治理軟警示觸發後，查核者須在交回單寫出責任邊界與行數增長原因；⛔ 不以該警示作為退回理由。
