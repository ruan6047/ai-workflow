...
- **身分以宣告欄位記錄，不追求平台驗證。** 「誰宣稱做了這件事」寫進欄位即可稽核；
  「證明他真的是他」屬目標 3，非必要。
- 該欄位**已經存在**：Project 的 owner 欄、commit 的 `Implemented-by` / `Planned-by` / `Requested-by`、
  review event 的 reviewer 字串。缺的是**要求它們被填、且填得夠細**，不是一套驗證機制。
- **不要寫看起來在驗證身分、實際恆真的條文。** 這一條不變——但正解是
  **改成宣告欄位**，不是把恆真性導出成 `structurally-vacuous` 再繼續假裝那是檢查。
- 代貼他人裁定時，**在留言內註明代擬代貼與授權來源**。那就是宣告欄位的散文形式。
