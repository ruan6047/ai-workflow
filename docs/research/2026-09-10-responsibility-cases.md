...
- **結果＝資料不足（不可判定）。** 逐欄列缺：**α 的期間未知、α 的「維護」動作分解未知；β 的期間未知、β 的「更新」動作分解未知。**⛔ 不給唯一權限答案、⛔ 不把空白讀成「R₂ 不限時皆可」。
- **反測 1（只補 β 的一個維度）**：補上「β 只在核可後逐字同步」⇒ **仍是資料不足，⛔ 不得宣稱相容**。α 的期間與動作分解**未被任何一句限定**，至少兩個完成模型都符合現有 α 文字且 ⛔ 未被排除：
  - 模型甲：α 的「維護」**涵蓋核可後的逐字同步** ⇒ 與 β 在對象、動作、期間三個維度**重疊** ⇒ 兩條對同一動作指派不同角色。
  - 模型乙：α 的「維護」**只含核可前起草** ⇒ 與 β 不相交。
  ⇒ 兩模型給相反結果而原文定不出哪一個 ⇒ 依 `core/conflict-resolution.md` §4 逐字「適用面任一維度、規範效果或量詞不足以定出結果時依 §6 判資料不足：⛔ 不把『沒寫』讀成『不相交』」**維持資料不足**。⚠️ **補一邊的資料 ⛔ 不能消除另一邊的未知。**
- **部分補齊的判準**：補了一部分資料後，**逐項問每個剩餘未知「它的任何一種補齊方式，有沒有可能改變結果」**。依 `core/conflict-resolution.md` §4 逐字「**相容**＝已知適用面不相交，或共同情境下的效果可並存」——**只要存在某一個維度，α 與 β 在該維度上都已知且值域不相交**，交集即為空；其他維度無論怎麼補，都 ⛔ 不可能把空交集變成非空 ⇒ 該剩餘未知**不影響結果**，此時應判**相容**、⛔ 不得判資料不足。反之，若**沒有任何一個維度是兩邊都已知的**，未知就可能翻轉結果 ⇒ 依 §6 判資料不足。⚠️ 這只涵蓋 §4 的「已知適用面不相交」那條路；「效果可並存」是另一條獨立的相容路徑，⛔ 不在本例示範。
- **反測 2（一個足以判相容的補齊構造，⛔ 不是必要條件）**：補上四句——α：「R₁ 的『維護』＝**起草**，且**只在規劃階段**」；β：「R₂ 的『更新』＝**逐字同步**，且**只在執行階段**」⇒ 期間不相交（規劃 vs 執行）**且**動作不相交（起草 vs 轉錄，`core/conflict-resolution.md` §1 四分）⇒ 轉為 case_disjoint（**相容**）。⚠️ **這四句是充分、⛔ 不是必要**——見反測 2a。
- **反測 2a（部分補齊：只刪一句，仍相容）**：從反測 2 的四句中**只刪 α 的「維護＝起草」**，保留 α「只在規劃階段」、β「更新＝逐字同步且只在執行階段」。α 的動作分解此時未知，但**兩邊的期間都已知且互斥**（規劃 vs 執行）⇒ 無論 α 的「維護」被補成起草、轉錄、落欄或四者任意組合，**期間交集恆為空** ⇒ 適用面仍不相交 ⇒ **仍是相容，⛔ 不得因「α 動作未知」就退回資料不足**。⚠️ 這 ⛔ 不是把未知讀成不相交，也 ⛔ 不是用角色不同判相容——分離軸是**原文已明寫**的規劃／執行互斥。
- **反測 2b（四句各刪其一，逐一實測）**：四個單一前提每次只刪一個，剩餘的分離軸逐項列出：
  | 刪掉的前提 | 剩餘的已知分離軸 | 結果 |
  |---|---|---|
  | α 的「維護＝起草」 | 期間：規劃 vs 執行 | 相容 |
  | α 的「只在規劃階段」 | 動作：起草 vs 逐字同步（§1 四分） | 相容 |
  | β 的「更新＝逐字同步」 | 期間：規劃 vs 執行 | 相容 |
  | β 的「只在執行階段」 | 動作：起草 vs 逐字同步（§1 四分） | 相容 |
  ⇒ **四種單刪都仍相容** ⇒ 「少補任一句就回到資料不足」這個全稱**不成立**。
- **反測 2c（刪到同一邊全未知才回資料不足）**：把**同一邊的兩句一起刪**（例如 α 的動作與期間都刪）⇒ 該邊在每個維度都未知 ⇒ **沒有任何一個維度是兩邊都已知的** ⇒ 依上述判準退回**資料不足**（即反測 1 的狀態）。⚠️ 跨邊刪兩句則不然：刪兩邊的動作 ⇒ 期間仍互斥 ⇒ 相容；刪兩邊的期間 ⇒ 動作仍互斥 ⇒ 相容。**關鍵是有沒有剩下一個兩邊都已知的分離軸，⛔ 不是刪了幾句。**
- **反測 3（位置與順序）**：在反測 1、反測 2、反測 2a 三個狀態下，各自把 α 移到 `core/`、β 移到 `stages/`，並把 β 寫在 α 之前 ⇒ **結果都不變**（反測 1 仍資料不足、反測 2 與 2a 仍相容）。差別只在**資料**，⛔ 不在位置或順序（`core/conflict-resolution.md` §7）。
- ⚠️ 本例的前一版逐字寫「補上『β 只在核可後逐字同步』一句 ⇒ 立刻變成 case_disjoint（相容）」——該反測**只補 β 就宣稱不相交**，違 §4 上引逐字，已依查核裁決 issuecomment-5639171763（`WF-002-R3.1-1`）更正。
- ⚠️ 本例的**再前一版**（補修 `WF-002-R3.1-1` 時新增）逐字寫「要翻成相容，**α 與 β 兩邊的期間與動作分解都必須寫明**」與「**少補任一句都回到資料不足**」——那是**過強的必要性宣稱**：只刪 α 的「維護＝起草」而保留兩邊期間互斥時，適用面仍不相交、仍應判相容（反測 2a），四種單刪亦皆相容（反測 2b）。該宣稱會讓方法的活案例**拒絕已經可以判定的資料**。已依查核裁決 issuecomment-5639400117（`WF-002-R3.3-1`）改為「四句是充分構造、⛔ 不是必要條件」，並補上部分補齊的逐項判準與反測 2a／2b／2c。

### case_exception_collision — 競爭例外

- 依 `core/conflict-resolution.md` §6 逐字，競爭例外的成立條件是「**兩條具名例外**同時命中且結果相斥」⇒ 本例必須有**兩個各自具名接回自己通則的例外**。
- **通則一** α（檔 X）：「R ⛔ 不寫 O；**具名例外＝β**。」　**例外一** β（檔 Y）：「R 在 c₁ **得寫** O（本條是 α 的具名例外，回指 α）。」
- **通則二** γ（檔 Z）：「R **必須寫** O；**具名例外＝δ**。」　**例外二** δ（檔 W）：「R 在 c₂ **⛔ 不得寫** O（本條是 γ 的具名例外，回指 γ）。」
- 共同情境＝同時滿足 c₁ 與 c₂。此時 **β 與 δ 兩條具名例外同時命中**：β 給「得寫 O」、δ 給「⛔ 不得寫 O」⇒ **規範效果相斥**。
- **結果＝競爭例外 ⇒ 回到各自來源比適用面。** β 的來源是 α（禁止通則）、δ 的來源是 γ（義務通則）；兩個來源的適用面在 c₁∩c₂ 交疊而**任一方都未接回另一方** ⇒ **仍相斥**。
- **仍相斥 ⇒ 判資料不足並上呈裁定**，⛔ 不由執行者選一邊、⛔ 不以「較新」「較具體」「所在目錄」判贏（§7）。
- **反測 1（刪例外一 β）**：α 的具名例外失去來源 ⇒ 回落禁令。c₁∩c₂ 下 α 禁寫、δ 也禁寫 ⇒ **兩邊同向、⛔ 無相斥** ⇒ 相容。
- **反測 2（刪例外二 δ）**：γ 的具名例外失去來源 ⇒ γ 無條件要求寫 O。c₁∩c₂ 下 β 允許寫、γ 要求寫 ⇒ **可同時履行** ⇒ 相容。
  ⇒ 反測 1 與 2 各刪一個來源都讓競爭消失 ⇒ **證明競爭確實來自「兩個具名例外」本身**，⛔ 不是來自任何單一條文。
- **反測 3（位置與順序）**：把 α／β／γ／δ 四條在 `core/`、`roles/`、`stages/`、`modules/` 之間任意互換目錄，或倒轉四條的先後 ⇒ **結果不變**（仍是競爭例外、仍判資料不足上呈）。目錄與順序 ⛔ 未進判準。
- ⚠️ 本例的前一版逐字是 γ「R 在 c₂ ⛔ 不寫 O，**此條無例外**」並自稱 γ 為「獨立禁令」——那只有 α→β **一條**具名例外，⛔ 不符 §6 的競爭例外定義；在該版本下 β 的「做」與 γ 的「不做」在共同情境相斥，依 §4／§6 應判**真衝突**而非競爭例外。已依查核裁決 issuecomment-5639171763（`WF-002-R3.1-2`）重建為兩個真正具名接回的例外，**⛔ 不是把獨立禁令改名冒充例外**。
...
{"id":5,"prior":"張力","result":"真衝突","dim":"動作","edit":["stages/planning.md §6"],"falsifier":"共同情境＝規劃階段、同一角色、同一對象（核心痛點）。規範效果互斥：planning §5「⛔ 不改核心痛點」是禁止，F-規劃-07「更正痛點」是義務 ⇒ 現行 conflict-resolution §4 逐字「必須做對 ⛔ 不得做」。⛔ 不以「兩邊都是絕對句」為由判真衝突（該判準已於本卡 25f6e97 刪除）。推翻＝舉出 F-規劃-07 的更正可由他人執行或在別的階段進行，使兩條可同時履行。"},
...
{"id":10,"prior":"待釐清","result":"相容","dim":"所有權粒度","edit":["roles/pm.md §2"],"falsifier":"⚠️ 需求方裁定 issuecomment-5638859953（B1 乙）：**基線期（f70c7ef）＝資料不足**，所有權粒度在當時文本定不出 per-file／per-entry；**現行＝相容**，本欄 result 記現行值。現行推導：共同情境＝執行者建 .wf/stages/<階段>.md 寫條目 x 後，PM 追加自己起草的索引 y（modules/pitfalls-13/module.md:51）。roles/pm.md §4 F-PM-12 逐字「維護 .wf/stages/<階段>.md 與卡面 notes **只加條目**；⛔ 不刪、**⛔ 不改寫**、⛔ 不加豁免鍵」是**追加**授權且自帶改寫禁令；同檔 §2 逐字「所有權按條目、⛔ 不按檔案」（需求方 issuecomment-5615910717 §② 核可的 conflict-resolution §3）使追加 y ＝修自己的產出物 ⇒ 兩條可同時履行。若改 x：F-PM-12「⛔ 不改寫」與 §2「⛔ 不代修」**兩邊都禁止、⛔ 無一邊要求** ⇒ ⛔ 無規範效果互斥。⚠️ 前一版本欄把 F-PM-12 讀成「改既有條目的義務」，該讀法已被同版本原文推翻（WF-002-R2.1-1），本次依裁定刪除。推翻＝證明 F-PM-12 在某版本逐字要求改既有條目（f70c7ef 與現行皆否），或證明基線期已有條文定出所有權粒度。"},
...
{"id":22,"prior":"相容","result":"真衝突","dim":"工具副作用","edit":["stages/closeout.md §5"],"falsifier":"⚠️ 需求方裁定 issuecomment-5638859953（B2 乙）：改判**真衝突**。共同情境＝卡進終態時的結案。規範效果互斥：stages/closeout.md §5 把「封存」列進 **PM 的人的動作清單**，而 core/verbs.md §1 move 寫欄逐字「進終態即關 issue 並封存」把它定為**動詞副作用**，且 core/conflict-resolution.md §1 逐字「工具副作用 ⛔ 不是人的動作…歸該動詞，⛔ 不歸跑它的人」⇒ 同一動作被同時指給人與工具。本卡核心痛點第 ② 項是已發生的實撞。消解＝在被裁定改動的那一條（closeout §5）就地修文並回指，⛔ 不動 core/verbs.md 與 core/glossary.md（經核對：verbs 該片語本就在 move 寫欄、glossary 封存詞條 ⛔ 未指派執行主體，兩者無需改動）。推翻＝證明 closeout §5 原句本就只表示「PM 促成封存發生（透過跑 move）」而 ⛔ 無互斥，則本次修文屬無反例改相容原文。"},
...
{"id":32,"prior":"相容","result":"相容","dim":"對象","edit":[],"falsifier":"若「不寫任何東西」含拋棄式 worktree 的檔案寫入即相斥；roles/reviewer.md §2 逐字「需驗證時走密封探針或容器」已具名例外。⚠️ 需求方裁定 issuecomment-5638859953（B3 乙）：本組 `edit: []` 指**本組未指定任何修文**，**⛔ 不表示共用條文逐字未動**——roles/reviewer.md:24 已因 #12／#34 於 63a5532 加入「（對象＝權威產出與狀態面；具名例外＝本卡 wf:note 候選留言…）」限定。AC4 ⛔ 不得再把該條描述為逐字未動。推翻＝證明在無該限定下 :24 的絕對禁令本就不涵蓋拋棄式 worktree 的檔案寫入。"},
