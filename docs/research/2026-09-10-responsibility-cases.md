...
- **反測 2（補齊足以定出結果的全部前提）**：要翻成相容，**α 與 β 兩邊的期間與動作分解都必須寫明**。補上四句——α：「R₁ 的『維護』＝**起草**，且**只在規劃階段**」；β：「R₂ 的『更新』＝**逐字同步**，且**只在執行階段**」⇒ 期間不相交（規劃 vs 執行）**且**動作不相交（起草 vs 轉錄，`core/conflict-resolution.md` §1 四分）⇒ 轉為 case_disjoint（**相容**）。⚠️ 少補任一句都回到資料不足。
- **反測 3（位置與順序）**：在反測 1 與反測 2 兩個狀態下，各自把 α 移到 `core/`、β 移到 `stages/`，並把 β 寫在 α 之前 ⇒ **結果都不變**（反測 1 仍資料不足、反測 2 仍相容）。差別只在**資料**，⛔ 不在位置或順序（`core/conflict-resolution.md` §7）。
