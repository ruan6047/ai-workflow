...
## Gemini 第五輪（被審 3d10bf8，留言 5542053766）：APPROVE

- 非擋建議：§十八補質詢（grilling）、單向門兩列。
...
- R3-01：D2 改為「清單項或撤銷卡，皆不在板」，§七 `open` 硬擋逐字對齊（§三、§七）。
- R4-01：H17 指定唯一居所 `roles/conduct-common.md` §1，§十一加一列。
- R4-02：§六「CLI 自己寫的留言」清單移除 `wf:log`；上輪只改 §十，又是兩處不同步。
- R4-03：§十八補合成表、模組 delta、設計閘（Design gate 的正式中文詞，PM 定）、驗證項目、清單收斂宣告五列，並在驗證項目列區分驗收條件／驗證項目／self_run。

## 第三十一輪（被審 2e168a4，留言 5541602774）

- R1-01：`wf:log` 改為人貼的純散文留言，CLI 不寫不讀，只以 URL 指向（§十、§十一、§十五）。⚠️ PM 判讀「單一寫入通道管卡面 JSON 與 Project 欄，留言本就有人貼的三種」；需求方 sign-off 時一併確認，未取得裁定前標在 §十。⛔ 沒加第八動詞、沒給任何動詞加旗標。
- R3-01：跨階退回列加 `stage_plan` 條件：含規劃→規劃／退回，否則→需求／退回；合成表按該卡 `stage_plan` 展開（§四）。
- R4-01：PR 粒度明列：0、1、2、3、4a、5、6、7 各一 PR，4b 每模組一 PR，全部完成才算第 4 步完成（§十三）。
- R4-02：§十八補待審清單、規格、資料有效性／平台委託、完整性、finding 五列，驗收條件的禁用同義詞加 AC。

## 第三十輪（被審 61556d6，留言 5541431393；首輪四題全審）

- R2-01：§十二「只能加嚴不能豁免」改成形狀「單向累加、無豁免鍵」；§一 glossary 行「只准用」改成「消費者＝」。
- R2-02：§六加 `grilling` 欄（T4 質詢紀錄的 `wf:log` 留言 URL），T4 離開規劃缺即 `move` 印；`open` 缺欄清單指回 §六必填時點＝建卡的欄。
- R3-01：§八定 `wf-ruling` JSON schema（kind 封閉六值，依 kind 的必要鍵：block 四欄、stop 三欄）；§六加 `blocked` 欄 {from, ruling}；§四阻塞與停止列、§七 `move` 印欄改引它。
- R4-01：§十三第 4 步分 4a 宣告／4b 條文回填，一模組一 PR，全部回填才算完成；deploy／maintenance 留帶日期的「待實例」；§十三開頭加每步完成點＝跨實體審 APPROVE＋sign-off。
- R4-02：§四、§十一、§十五寫明預設＝換人、需求方可否決。
- R4-03：§一加 `ADOPTION.md`，§二加其固定節與 60 行上限。
- R4-04：§十八補分支、被審 SHA、合併方式、寫入契約、副作用入口、退回理由六列；「清單收斂宣告」「規格版本」「阻塞資訊」骨架未逐字使用，不加；「Design gate」是 §十一落點名，非正文用詞。

## 第二十九輪（被審 0a01400，留言 5541306745）

- R4-01：可達性測試矩陣隨被測物累加：第 1 步只測無模組，第 4 步每個帶 delta 的模組同 PR 加自己的案例，最後一個加全部啟用（§四、§十三）。
- R4-02：新增 `core/platform.md` 作 P1–P5 條文居所，ruleset 與 CI 為其執行 artifact；D1–D4 居所寫明為 `core/verbs.md` §寫入契約；core 八檔→九檔（§一、§三、§十三）。
- R4-03：§十八補失誤登記、復活條件、翻案把手三列；「驗證項目」「清單收斂」骨架未逐字使用，不加。

## 第二十八輪（被審 6ec6deb，留言 5541006615）

- R2-01：§二兩句書寫紀律、§十二兩句操作流程改寫成形狀（節次上限、條文形狀、候選留言形狀、回看清單形狀），條文落點指到 `roles/conduct-common.md` §書寫／§注意事項與 `roles/pm.md` §4。

## 第二十七輪（被審 2376925，留言 5540862220）

- R3-01：§四釘死三個模組狀態的 delta（進邊＋可達結案的出邊），並加通則「模組加狀態必附出邊」；CI 可達性改為對無模組／單一模組／全部啟用各跑一次。
- R3-02：`stage_plan` 結構有效性進 D3：8 階段序列的子序列、不重複、必含需求／執行／審核／結案。T2+ 缺規劃維持印（PM 判），不進硬擋。

## PM 自審（c790d29 後）

- 同類漏寫：D3「解析失敗整卡拒」寫明 CLI 全動詞，但 §七 `move`／`notes`／`brief`／`snapshot` 列沒寫；補上。

## 第二十六輪（被審 41e37b0，留言 5540659918）

- R3-01：§七 `edit` 硬擋補 `--set source_sha=` 不在遠端、已給的 `--ruling` URL 不存在（都是 D4 既有項，只是 `edit` 列漏寫）。

## 第二十五輪（被審 413505f，留言 5540580455）

- R1-01：db-contract 啟用條件改為只看 `modules.json`（與其他專案層模組同形）；契約檔缺席改成已啟用模組自己的完整性提示。§九不再有 AND。
