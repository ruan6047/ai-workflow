...
依此重過 §七的硬擋：CLI 層只准三類——**資料有效性**（JSON 合法、鍵集合封閉、轉移在合成表內、SHA 在遠端存在、裁定 URL 存在）、**寫入順序**（檢查在第一次遠端寫入前、寫後回讀）、**平台委託**（ruleset、CI、repo 設定）。00 §二裡屬「判讀」的兩條降級：H13 的「APPROVE 與 open blocking 並存」改為印警示；H18 四停下條件改為印，由 PM 判。硬擋以本檔 §七 的表為準；`extract/00-consolidated.md` §二（19）與 `07-conflicts.md`（20）是歷史計數，⛔ 不再更新。
