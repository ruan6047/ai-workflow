...
**第零條（README 心智模型第一行）**：CLI 提供資訊清單，AI 判斷；CLI 只確認清單有沒有填，⛔ 不做內容判讀。舊 ROADMAP 目標 1「有機械執行者擋下才算達成」廢止。

依此重過 §七的硬擋：CLI 層只准三類——**資料有效性**（JSON 合法、鍵集合封閉、轉移在合成表內、SHA 在遠端存在、裁定 URL 存在）、**寫入順序**（檢查在第一次遠端寫入前、寫後回讀）、**平台委託**（ruleset、CI、repo 設定）。00 §二裡屬「判讀」的兩條降級：H13 的「APPROVE 與 open blocking 並存」改為印警示；H18 四停下條件改為印，由 PM 判。硬擋淨數 18。
...
4. 守衛：**預設不做**。只有處理手段落在「不可逆或平台層事故」時，需求方才可裁定寫成硬擋（§二 20 條的判準），且必須指得出執行者（ruleset、CI、CLI 拒收）。引用次數多⛔ 不是守衛化的理由——上一版的 CLI 就是這樣長出來的。
