...
③ `cli/src/wf/verbs/_common.py` 的 `壞區塊只記錄，⛔ 不擋（派工單 §5）` 把居所引用直接刪掉未改指，與 `review.py` 改指 `core/verbs.md` 兩套標準；真居所在 `core/verbs.md` §2「`snapshot` 例外＝⛔ 不拒、依 §1 記入本機輸出並續跑」。
...
⑦ `test_gh_scope.py` 的 docstring 守衛只覆蓋五檔白名單；本片 25 處 docstring 改動只有 1 處在守衛內，且 `compose/project_config.py` 的 `§` 出現次數由 2 降為 1、餘裕歸零。修法＝白名單改 `rglob('*.py')` 全掃。
