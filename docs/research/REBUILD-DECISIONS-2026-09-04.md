...
- **回看（2026-09-10 第三批）：三輪研究的結論，以及一件該開卡的。** 研究全程唯讀、六隻代理（4 Claude 子代理、`gpt-6-astra`、`gemini-3.8-flash-high`）分三輪跑，原始報告在 session scratchpad，⛔ 未進 repo。
  ...
  **⑤ 「加進清單」買不到遵守（實測，這是本次研究最重的結論）。** 母體＝今天 6 張卡 13 份交回單 **291 條 `note_responses`**。`followed` 的實質率（text 含指令／rc／路徑才算）在**同一張卡、同一份清單**下由 **8% 走到 57%**——變數只有「被退回一次」。跨交回單比條數 vs 實質率：卡為獨立單位（N=7）Spearman **−0.374、p=0.42 ⇒ 依 F-research-01 逐字「不顯著⇒不可判定」**。⚠️ **決定性反證在同卡內**：`WF-002` it=0 清單 19 條 → 實質率 8%；it=1 清單 **24 條（更長）** → **57%**，方向與「短清單比較好」**相反**。
  ...
  **⑦ 需求方 2026-09-10 的「權重／情境化」猜想（逐字「如果清單裡配置權重或者增加情境 似 skill 的文本架構」）：結案為「已實現一半、另一半不值」。**
  ...
  - **權重零效應**：同一條在不同位置**答案完全一樣**（`F-執行者-10` 在 pos 10／15／19 都是 `not_applicable`；`F-執行者-01` 在 pos 1／6／10 都是 `followed`）。表面的「尾端衰退」是**條目身分效應、⛔ 不是位置效應**。且配權重三條路全堵死：寫條目上＝T4／PM 每卡配＝PM 做內容判讀／專案層配撞 `core/verbs.md` §3 逐字「專案與卡面只能加嚴：⛔ 不得刪除或改寫上游條目」。
