...
## 跨家族覆核（`gpt-6-astra`，2026-09-12）：五條全部要收窄，另找到三處同族漏網

⚠️ **本節是覆核結果的登記，⛔ 不是裁定。** 上一節「回看清單補五條」的每一條都標了「⛔ 未經跨家族覆核」；這一輪補上。**五條沒有一條被全盤接受。**

⚠️ 覆核者的自陳限制：它的 `gh api` **連線失敗**，故凡依賴 `#311` 原留言的判一律 `不可判定`；它用人工樣本重現現象，並逐字寫「**⛔ 不替代 `#311` 的逐字實測**」。

### 五條的判

| # | 判 | 收窄逐字 |
|---|---|---|
| 一 `--source-sha` 靜默丟棄 | **站得住，須收窄措辭** | 「合法邊、合法卡、SHA 存在時，需求／規劃交回收到 SHA 卻保留舊值；**進執行則清空**；均無『旗標未採用』提示。**⛔ 並非所有輸入都 rc=0**」 |
| 二 `誰填` 表粒度 | **站得住，但有一半站不住** | 「六欄沒有相同編輯規則；但『**六欄完全不同**』過度概括，且 **PM 由沉默推成 `owner` ⛔ 不可 edit 站不住**」 |
| 三 撞號線性成長 | **站得住** | 「⛔ 不能據此推算真實事故率或使用者損失」 |
| 四 `snapshot` 靜默 | **不可判定（`#311` 原事件）** | 人工樣本重現成立；`#311` 原事件因遠端不可讀而不可判定 |
| 五 `move --ruling` 訊息錯 | **不可判定（原事件）** | 同上 |

### ⚠️ PM 的第 15 次自錯：把「某個欄位失去區分」寫成「整個輸出分不開」

覆核者逐字：

> **PM 漏列一行重要輸出：`wf-return JSON 解析失敗`。** `_common.py` 的 `comment_blocks` 雖把 `blocks` 中的值壓成 `(False, None)`，**但仍透過 `errors` 保存原因**；`move._ruling_prints` 也會印它。因此「**`blocks` 子欄失去區分**」成立，「**整個 `comment_blocks` 回傳值或 `move` 全輸出無法區分**」站不住。
...
> 自算 `5/13 = 0.384615384615`。若 13 與 5 的分類及計數正確，**只能說：已登記的錯誤中，這五次由別人先指出**。分母⛔ 不是所有判斷／所有產出，**也⛔ 不知道尚未被發現的錯誤**。⛔ 不能換算成「PM 38.46% 會錯」或「自查召回率 61.54%」。
> 第 14 次也⛔ 不能逕自把外部發現數加一、推成 6/14。

⇒ **PM 先前在報告與交接文件裡把它當成比例引用，⛔ 不成立。**

### ⚠️ 第 14 次（額度「六天後才回」）的判：**不可判定**

覆核者逐字：

> 派工單給的 stderr 原文含 `...`，**本身⛔ 不是完整底稿**；本輪也未取得 23:58 的原呼叫記錄、帳號／模型／端點與記憶檔。因此其歷史實測 **不可判定**，**⛔ 不把使用者提供的重述冒稱我已複驗**。
> 若「同一帳號、同一模型與同一服務路徑」的最小探針同日 rc=0、回 `QUOTA-OK` 為真，足以推翻「該路徑六天內都不能再成功呼叫」；**⛔ 不保證大任務也不會限流、⛔ 不代表額度全面恢復**，更⛔ 不解釋服務端為何先前顯示那個日期。

---

## K2 · 同族漏網三處（覆核者實跑，附負控）

掃描母體＝本次 HEAD 的 `cli/src/wf/` 全部 **26 個 Python 檔**（附 `SCAN_MANIFEST`）。覆核者逐字：「這是候選搜尋與人工覆核，**⛔ 不是能證明所有同形問題均已找到的完備偵測器**。」

1. **`brief.py` 的 `_remote` → `_baseline`**：**沒有分支／分支讀取失敗／merge-base 讀取失敗**，三種都變成同一句「無分支，基線＝main 頭」。負控＝正常分支能印真合併基底。
2. **`brief.py` 的 `_previous_findings`**：**沒有前輪／前輪明確 `findings=[]`／前輪缺 `findings`**，三種都印「無前輪」。
   ⚠️ 逐字：「**有前輪且 `findings` 為空⛔ 不等於無前輪**…目前文案**抹掉了可供 AI 判的存在／完整性差異**。」
3. **`edit.py` 的 tier 降級分支丟掉 `comment_blocks.errors`**：**無區塊／壞 `ruling` JSON** 都只印「`tier` 降級而缺 `--ruling` 或 `kind≠tier_change`」。⚠️ 覆核者逐字：「與第五條不同，**這個消費端真的⛔ 不再印解析失敗原因**。」

⇒ 連同已登記的三條（撞號、`comment_blocks`、`snapshot`），**同族實例至少六處**。

---

## K3 · 這一族錯誤的共同特徵，以及它能機械化到哪裡

覆核者給的特徵逐字：

> **把較弱的觀察提升成較強的主張，卻⛔ 沒保留能區分反例的條件、來源或完整輸出。**
> 這是**可做局部機械測試的錯誤形狀**，⛔ 不是已證明 14 件全部同源的心理診斷。

六個落點，**每個都附機械能力的界線**：

| 形狀 | 可測部分 | ⚠️ 界線 |
|---|---|---|
| 引錯檔／節、引文不存在 | 對指定版本做逐字包含與唯一命中檢查；真引文／假檔名雙向對照 | 能判引文定位，⛔ 不能判授權如何分工 |
| 移除前提（如「執行者失聯時」） | 同時保存完整來源條目與被引片段，列出刪掉的上下文 | 可列差異；哪些前提改變含義仍由 AI 判，**⛔ 不能把所有短引文視為違規** |
| 子字串計數取代實際區塊 | 用同一圍欄解析器；造「散文提到」與「真區塊」兩組輸入 | 可驗結構；**⛔ 不能把無區塊散文的立場自動判成不存在** |
| rc=0 被當全部後置條件成立 | 保留 rc、完整 stdout／stderr；有寫入則保存回讀與差異 | 可驗狀態是否相同，**⛔ 無法替任意業務目標判完成** |
| 不同原因落成相同提示 | 在 parser 與消費端建 `absent`／`invalid`／`valid` 成對樣本 | 可釘住已知案例；**所有多對一映射⛔ 並非都有害，⛔ 不能只查 `return None` 就定罪** |
| 要排除四案卻未提供材料 | 派工清單列材料識別與可讀來源，檢查有填、可存取 | 能確認附件在不在，**⛔ 不能靠數出四個檔就認證內容足夠** |

⚠️ **落點的居所**：覆核者逐字建議放在「呼叫服務的**包裝層**」——保存錯誤為「當時服務回報」，另存相同路徑的低成本探針時間、退出碼與回應；後續成功時列出與舊主張衝突的證據，**交 AI 更新記憶**。
⇒ 逐字結論：「**這符合第零條，⛔ 無須把內容裁判塞進 `wf` CLI。**」

## ⚠️ 覆核者把自己也當反例

> **這一輪本身也是反例：** 我初次測第五條選了非法轉移；有 D1 與合法邊對照後才能辨識是**探針前提錯**，而非功能未重現。**跨家族⛔ 不會消除這類錯誤，保留反例、完整輸出與版本才讓它可被抓到。**

## ⚠️ 覆核者對「跨家族查核」價值主張的收窄

> **⛔ 沒有證據顯示我有其他家沒有的能力。** 本輪使用的檔案讀取、Git、Python 與替身實跑都可重做；**遠端連線反而受限**。**模型家族差異只能提供另一個判讀來源，⛔ 不能當能力優越或結論正確的證據。**

並逐字補：「`core/tiers.md` §1 的 T4 確實要求兩位查核者彼此不同家族，但**不同家族⛔ 不等於統計獨立**，也⛔ 不表示本回報已讓五條全部通過。」
