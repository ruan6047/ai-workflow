...
>
> **但「錯誤引用」不等於「正確版本在別處」。** 本卡曾把引數正規化的歸屬指向 #23；**#23 已於其交付版（`50021ce`）§4.1b／§10 明文拒絕承接**：它逐一追碼六個承接動詞的引數，裁定「承接的六個動詞裡，沒有任何一個引數需要檔案系統路徑正規化」，因而**不定義**也**不引用**任何 CLI 路徑正規化器。**故 CLI 路徑引數的正規化目前無人擁有。這是事實陳述，不是待辦——沒有人欠這個函式。**
>
...
| 57 | 本檔 §9.9 的自檢落成 repo 內腳本並掛 CI，對本檔執行 | 退出碼 **0**；且對**人為植入**的六種變異退出碼 **非 0**：(a) 「f-string 取值部含反斜線」（PEP 701，3.12+ 才合法）、(b) **`type _MutAlias = int`（PEP 695 型別別名語句，3.12+ 才合法，與 f-string 無關）**、(c) 「刪去一個探針區塊」、(c2) 「新增一個未登記區塊」、(c3) 「刪去 `probe-blocks` 登記行」、(c4) 「植入互相矛盾的第二行登記」。(b) 的作用是釘住閘門**不是只認得已知那一種形狀**——它必須攔下任意高於下限的語法；(c)–(c4) 是同一個逐檔登記機制的四個分支，作用是釘住它沒有靜默放行路徑（實跑輸出見 §9.9.4）。**R6 的 (e)(f) 兩個變異隨執行路徑一併移除**，理由見 §9.9.4 |
| 57a | 自檢執行環境**找不到**版本 ≤ 3.11 的直譯器 | 退出碼 **非 0**，訊息為「可攜性宣稱無從佐證」；**不得**退回用執行中的直譯器編譯而靜默 PASS（§9.9.1-C） |
| 57b | 自檢對**本檔以外**的文件執行（跨檔一般性） | 對一份**有** `probe-blocks` 登記、區塊皆可被下限直譯器剖析的文件退出碼 **0**；對同一份**無**登記者退出碼 **非 0** 且違例恰為登記缺失一筆（§9.9.7；R6 新增，R7 隨收窄改寫——原列要求「四支全部實際執行」及三個執行期約定，已隨執行路徑移除） |
| 57c | **自檢對一份「唯一區塊會寫檔」的第三方文件執行**（R4-001 的直接處置，R7 新增） | 該區塊**不得被執行**：斷言其副作用（檔案 `side-effect-created`）**不存在**。**不得**以「退出碼非 0」當作斷言——語法合法、登記正確的文件本來就該 PASS，收窄要保證的是它的內容無從發生，不是把它判 FAIL（§9.9.4 的 (g)）。**另須結構性斷言**：自檢腳本自身的 AST 中不存在對 `exec`／`eval`／`compile`／`__import__` 的呼叫，且 `subprocess` 的指令列只來自寫死常數——防的是日後有人把執行路徑加回來 |
...
# 需外部資源：網路與 gh 憑證（list_items 打 GitHub API）。這是給讀者的散文註解，
# 不是機制——§9.9 的自檢不讀它，也不執行任何區塊（R4-001）。
...
# 需外部資源：網路與 gh 憑證（list_items 打 GitHub API）。散文註解，非機制（同 §9.7）。
...
**這份輸出是怎麼產生的（R7 起改變）**：由**人直接執行**本區塊，指令為 `sed -n '<起>,<訖>p' docs/WF_RESOURCE_WRITESET1.md | /usr/bin/python3`（行號取自 §9.9.5 的自檢輸出，即 `L804-891`），**跑在 3.9.6——也就是宣稱的可攜下限本身**，退出碼 `0`，2026-08-12 09:43 +0800 複跑，內容與上列**逐行相符**（上列僅修剪了兩行的行末空白，無其他差異）。R6 曾由 §9.9 的自檢巢狀執行它，**R7 收窄後自檢不再執行任何區塊**（§9.9.1-D），故本節的執行證據是一次性的人工取證，不是持續的機械保證；把它掛成 CI 的獨立步驟歸衍生卡（§9.9.6）。順帶一提，這次的執行環境比 R6 那次**更貼近宣稱**：R6 是在 3.14 上跑的。
...
下列程式**抽出目標文件全部 `python` 圍籬區塊**，把每一段交給一個**版本不高於宣稱可攜下限的真實直譯器**編譯，並核對文件自己登記的區塊數。**它不執行任何區塊**（R4-001 的處置，見下），因此它自身被抽到時也不再需要任何防遞迴的標記。**無網路、無 `wf_cli` 依賴；退出碼非 0 即失敗，可直接掛 CI。**
...
>
> **R6 的處置**：R4 的自檢**只在本檔上驗過**，[#23](https://github.com/ruan6047/ai-workflow/issues/23) 第一次把它原樣指向別的文件就踩到三個缺陷（假陽性、靜默的覆蓋損失、恆真檢查）。R6 把三者連同兩個同族的隱性假設一併關掉，並在本檔以外的文件上實跑取證。**其中「未登記即執行」這一項在 R7 被撤除**，理由見下一段；R6 為它寫的證據段落已隨之改寫（§9.9.7）。
>
> **R4-001 的處置（本輪 R7）：把契約收窄成「只編譯，不執行」。** R6 把「執行」翻成預設（未登記即執行），而「採用本自檢的文件其 `python` 區塊必須唯讀」只是一句**文字要求**——程式對任何未標記的區塊照跑不誤。查核者把那句文字變成了可執行的利用：一份只含 `pathlib.Path("side-effect-created").write_text(...)` 的第三方文件，在自檢下印出「已執行……違例 0 筆……[裁決] PASS」，同時**真的把檔案寫了出來**。**作者的自我宣告不是安全邊界**，而本卡只宣告一個 markdown 檔、做不出可強制的隔離邊界（可行性實測與論證見 §9.9.9），故本輪走查核者 disposition 的**第 2 條**：**收窄契約——本自檢不執行任何文件的任何區塊，且不提供任何「執行」的開關或旗標**。代價逐項寫在 §9.9.6（§9.8 的裁決不再被自動複現）、§9.9.9（對 #23 沿用聲明的影響）與 §13。

**逐檔登記（R6 起，本輪保留）。** 自檢不對目標文件做任何推斷，唯一需要文件配合的事是**顯式登記區塊數**，且**行首錨定**，不做「原始碼含某子字串」那種寬鬆比對：

- 文件層 `<!-- probe-blocks: N -->`（本檔的登記行即下一行，markdown 不渲染）：宣告本檔登記幾個探針區塊。**缺登記即 fail-closed**，理由見 §9.9.1-E。

**R6 的兩個區塊層標記（`# probe-self`、`# probe-requires:`）本輪一併撤除。** 它們的唯一用途是決定「執行或跳過」，而本輪不再執行任何區塊；留著一個不再被讀的標記比沒有標記更危險——**它看起來像機制**，而查核者明文禁止把作者自我宣告的 `probe-requires` 當安全邊界，最乾淨的處置是讓它不存在。§9.7／§9.7b「需要網路與 `gh` 憑證」的事實改以**純散文註解**寫在區塊首行：讀者看得見，程式不讀它，也不因它而改變任何行為。

<!-- probe-blocks: 4 -->

```python
# §9.9 探針自檢（靜態）：抽出目標文件全部 python 區塊，交給一個版本**不高於**宣稱可攜下限的
# 真實直譯器逐一編譯，並核對文件層 probe-blocks 登記。找不到這種直譯器即 fail-closed。
#
# **本工具不執行任何區塊。** 對區塊原始碼只做兩件事：(1) 在本行程 ast.parse（剖析，不求值）、
# (2) 交給閘門直譯器的 stdin，由它 ast.parse。沒有 exec／eval／compile(mode="exec")／runpy／
# import，也不把區塊內容當程式啟動；因此目標文件可以是**不受信任**的（R4-001，§9.9.1-D）。
...
PARSE = "import ast, sys; ast.parse(sys.stdin.read())"   # 剖析，不求值
...
print(f"探針自檢（靜態）：{DOC}；抽出 python 區塊 {len(probes)} 個；執行直譯器 {run_ver}；宣稱可攜下限 {floor}")
print("執行覆蓋：0——本工具只編譯，不執行任何區塊（R4-001，§9.9.1-D）")
if gate is None:
...
for start, end, src in probes:
    ...
    if gate is not None:
        r = subprocess.run([gate[1], "-c", PARSE], input=src, capture_output=True, text=True)
        if r.returncode:
        ...
        note.append("閘門編譯 OK")
    try:
        ast.parse(src, f"{DOC}:{start}")           # 剖析，不求值；不建立可執行物件
    except SyntaxError as exc:
        failures.append((label, f"執行直譯器剖析失敗：{exc}")); print(f"  {label}：剖析失敗"); continue
    note.append("執行直譯器亦剖析通過")
    print(f"  {label}：{'；'.join(note)}")
print(f"違例 {len(failures)} 筆")
for label, msg in failures: print(f"  [FAIL] {label}：{msg}")
print(f"[裁決] {'PASS' if not failures else 'FAIL'}（靜態：閘門編譯＋區塊數登記；未執行任何區塊）")
sys.exit(1 if failures else 0)
```

#### 9.9.1 它檢查什麼，為什麼是這五件

A、B、C 是 R4 定下的可攜性主幹，未改；D 是 R7 的收窄（不執行），E 是 R6 為了讓它在**別人的文件**上也成立而改的逐檔登記。**R6 的 D／F／G 三項（`SystemExit` 裁決、執行期環境隔離、區塊層標記）隨執行一併撤除**，其歷史記在 §9.9.1-D 的末段——它們處理的都是「執行探針時該怎麼做」，而本輪的答案是不執行。
...
**D. 不執行任何區塊**（R7 對 R4-001 的處置；§9.6 第 57c 列）。自檢對區塊原始碼只有兩條路徑：在本行程 `ast.parse`，以及交給閘門直譯器的 stdin 由它 `ast.parse`。**兩者都只剖析、不求值**；程式裡沒有 `exec`、`eval`、`compile(..., "exec")`、`runpy`、也不把區塊內容寫成檔案再啟動。閘門的指令列是**寫死的常數 `PARSE`**，不由區塊內容決定。

**為什麼這是必要的，而不是保守。** R6 的設計是「未登記即執行」，安全性由一句文字承擔——§9.9.1-D（舊）與 §13 都寫著「採用本自檢的文件，其 `python` 區塊必須是唯讀的」。**那句話沒有執行者**：程式無從判斷一段碼會不會寫檔，作者也不必誠實。查核者據此建的利用是三行（§9.9.4 情境 (g)）。更根本的是**信任方向錯了**：自檢是拿來驗**別人**的文件的，而「別人的文件」正是它唯一的輸入；把「輸入是善意的」當前提，等於這個工具在自己的威脅模型裡不成立。

**為什麼不是「加一個 `--execute` 旗標」。** 那只是把同一句沒有執行者的話搬到操作紀律上，而交付輸出勢必是開著旗標跑出來的——示範的模式就會是不安全的那個。**本輪不提供任何開關**：要執行，就是使用者自己去跑那支探針，此時他明確知道自己在執行誰的碼。

**這一條的執行者不是這段文字，是 §9.9.4 情境 (g)**：把查核者的利用文件原樣跑一次，斷言 `side-effect-created` **不存在**。宣稱由測試釘住，不由句子釘住。

**代價（明列，不藏）**：(1) §9.8 的 `[裁決] PASS` 不再被自檢自動複現，其模組層 `ok` 與探針退出碼**都不再被任何自動機制讀取**；本檔改以直接執行取證（§9.8 節末），CI 落地時應成為獨立的一步（§9.9.6）。(2) #23 §4.4.1 兩項具名適配（`SystemExit(0)` 視為通過、以「需憑證」決定是否執行）**失去標的**——沒有執行就沒有裁決約定與跳過判準可談，影響評估見 §9.9.9。

**R6 的 D／F／G 三項一併撤除，記在此處免得被無痕改寫**：`SystemExit(0)` 視為乾淨結束、`KeyboardInterrupt` 往外拋（原 D）；`__name__` 設為 `"__main__"`、執行期隔離 `sys.argv`（原 F）；`# probe-self`／`# probe-requires:` 行首錨定（原 G）。三者在 R6 都是對的修正，處理的都是「執行時該怎麼做」；本輪的答案是不執行，故它們的機制、以及 §9.6 原第 57 列的變異 (e)(f)、原 §9.9.7 (2) 的第三份最小文件，全部隨之移除而非保留成空殼。
...
**本節現在宣稱的射程，一句話**：對任意（含不受信任的）markdown 文件，本自檢機械保證的是「每個 `python` 圍籬區塊都能被一個真實的 ≤ 3.11 直譯器剖析」與「區塊數與文件登記相符」，**其餘一律不保證**——不保證探針跑得起來、不保證它們的斷言成立、不保證它們今天仍與正文的數字一致。**編譯閘門對語法類的可攜性風險是完整的，對語意類的則完全看不見**（#23 §4.4.1 已就其 argparse 私有介面的探針點名這一點：編譯閘門對它是必要而非充分）。那半邊現在**明確不在本機制內**，歸屬見 §9.9.6 與 §9.9.9，不留在文字裡假裝有人管。
...
自檢若只在修好之後跑一次，證明不了它抓得到東西。以 **R2 交付版**（`cb6028f`）的檔案為輸入，用 **R7 這一版**的自檢重跑：

```text
探針自檢（靜態）：r2.md；抽出 python 區塊 3 個；執行直譯器 3.14.3；宣稱可攜下限 3.11
執行覆蓋：0——本工具只編譯，不執行任何區塊（R4-001，§9.9.1-D）
可攜門檻：3.9.6（/usr/bin/python3）｜模式 stricter（低於下限：嚴於宣稱而非等價，見 §9.9.1）
區塊數登記：文件未登記區塊數
  L609-664：閘門編譯 OK；執行直譯器亦剖析通過
  L692-733：閘門編譯失敗
  L768-855：閘門編譯 OK；執行直譯器亦剖析通過
違例 2 筆
...
[裁決] FAIL（靜態：閘門編譯＋區塊數登記；未執行任何區塊）
```

（退出碼 `1`。）

兩筆逐一對：**第二筆是 R2-001 本身**——閘門在沒有任何「f-string」專用知識的情況下抓到了同一段，它只是把那段碼丟給 3.9.6 去剖析，與查核者當初手動 `sed -n '694,733p' | python3` 撞到的是同一處。第一筆是該檔沒有 `probe-blocks` 登記行（R2 版連 §9.9 都還不存在）。

**與 R6 版跑同一份輸入相比少了一筆，而且少得對。** R6 版在此還會多印 `[FAIL] L609-664：執行拋錯：ModuleNotFoundError: No module named 'wf_cli'`——那是它「未登記即執行」把 §9.7 真的跑起來、在無 `wf_cli` 的環境下如實失敗。R7 不執行，該筆消失。**這就是收窄的代價具體長什麼樣**：少掉的是「探針跑不跑得起來」這類**語意**訊號，保留的是**語法可攜性**訊號。本節要防的 R2-001 屬於後者，故防護力未因收窄而下降；語意那半邊的歸屬見 §9.9.6，**它現在是明確的缺口，不是默認有人管**。

#### 9.9.4 變異與情境：八個分支案例，八次非 0（§9.6 第 57／57a／57b／57c 列）

**先講清楚這八格涵蓋的是什麼**，免得數字被誤讀成「八個獨立機制」（查核者對前一版的八個變異提過同一點）：本節共八格，落在**三個**機制上——(a)(b) 是同一道語法閘門的兩個分支案例，(c)(c2)(c3)(c4) 是同一個逐檔登記機制的四個分支案例，(57a) 是該閘門找不到直譯器時的退化分支，(g) 是 R7 新增的不執行邊界。**分支案例多不等於機制多**；列多個分支是因為同一機制的不同路徑各自可能單獨壞掉（`<` 對 `!=`、缺登記對矛盾登記），不是為了讓覆蓋數字好看。

(a)(b) 植入本檔 §9.7b 探針的首行之後。(a)–(c4) 全部以同一支自檢、同一條命令跑，行號取自各變異檔自身：

```text
變異 (a) f-string 取值部含反斜線（PEP 701，3.12+ 才合法）　　退出碼 1
  [FAIL] L720-767：閘門 3.9.6 編譯失敗：SyntaxError: f-string expression part cannot include a backslash；feature_version=3.11 未複現……

變異 (b) type _MutAlias = int（PEP 695 型別別名語句，3.12+ 才合法，與 f-string 無關）　　退出碼 1
  [FAIL] L720-767：閘門 3.9.6 編譯失敗：SyntaxError: invalid syntax；feature_version=3.11 亦拒收 → 確屬下限違例
...
**(c2)–(c4) 是 R6 新增，針對「只在自己文件上成立」那個病**：(c2) 釘住 `!=` 而非 `<`；(c3)(c4) 釘住「缺登記／登記矛盾一律 fail-closed」，否則 §9.9.1-E 的登記機制自己就有一條靜默放行路徑。

**R6 的變異 (e)（探針以非 0 退出）與 (f)（拿掉 `probe-requires` 登記）本輪移除**，因為它們測的是執行路徑，而執行路徑已不存在。留著它們就會出現一種糟糕的東西：**測試通過但被測機制已被刪掉**。取代它們的是下面的 (g)。

**(g)：不執行邊界的執行者（R7 新增，§9.6 第 57c 列）。** 這一格不是植入變異，而是**把查核者的利用原樣跑一次**——一份 `probe-blocks: 1`、唯一區塊為 `pathlib.Path("side-effect-created").write_text("proves execution")` 的第三方文件：
...
實際執行 1 個離線探針；違例 0 筆
[裁決] PASS
$ ls -l side-effect-created
-rw-r--r--  1 …  16 …  side-effect-created        ← 檔案被建立，而裁決是 PASS

── R7（本輪）　退出碼 0
執行覆蓋：0——本工具只編譯，不執行任何區塊（R4-001，§9.9.1-D）
區塊數登記：文件登記 1 個
  L6-7：閘門編譯 OK；執行直譯器亦剖析通過
違例 0 筆
[裁決] PASS（靜態：閘門編譯＋區塊數登記；未執行任何區塊）
$ ls -l side-effect-created
ls: side-effect-created: No such file or directory   ← 未建立
```

**兩邊都是 PASS 且退出碼都是 0，差別全在有沒有那個檔案**——所以 (g) 的斷言必須是「`side-effect-created` 不存在」，不能是「退出碼非 0」。這一點值得停一下：**收窄不是把利用文件判成 FAIL**。那份文件在語法上完全合法、登記也對，靜態檢查沒有理由判它 FAIL；判 FAIL 反而是又一次「用猜的代替判定」（沒有任何程式能可靠地認出「這段碼會寫檔」）。收窄要做的是**讓那份文件的內容無從發生**，而 PASS 這個字現在只承諾它承諾得起的事——裁決行本身就印著「未執行任何區塊」。

**同時釘住結構面**（§9.6 第 57c 列第二半）：對自檢腳本自身做 AST 掃描，斷言不存在對 `exec`／`eval`／`compile`／`__import__` 的呼叫，且 `subprocess.run` 的指令列參數只來自寫死常數。本輪實測輸出為「直接呼叫 exec/eval/compile/`__import__`：無」。**這條比 (g) 弱但方向不同**：(g) 證明一個具體利用不發生，結構斷言防的是日後有人把執行路徑加回來。

#### 9.9.5 在本次交付版上的輸出

```text
探針自檢（靜態）：docs/WF_RESOURCE_WRITESET1.md；抽出 python 區塊 4 個；執行直譯器 3.14.3；宣稱可攜下限 3.11
執行覆蓋：0——本工具只編譯，不執行任何區塊（R4-001，§9.9.1-D）
可攜門檻：3.9.6（/usr/bin/python3）｜模式 stricter（低於下限：嚴於宣稱而非等價，見 §9.9.1）
區塊數登記：文件登記 4 個
  L631-688：閘門編譯 OK；執行直譯器亦剖析通過
  L720-766：閘門編譯 OK；執行直譯器亦剖析通過
  L805-892：閘門編譯 OK；執行直譯器亦剖析通過
  L942-1064：閘門編譯 OK；執行直譯器亦剖析通過
違例 0 筆
[裁決] PASS（靜態：閘門編譯＋區塊數登記；未執行任何區塊）
```

（退出碼 `0`。輸出中不再有任何巢狀印出的探針內容——那是本輪收窄最直接的外顯差異。）

四個區塊即 §9.7（`L631-688`）、§9.7b（`L720-766`）、§9.8（`L805-892`）、§9.9 自身（`L942-1064`）。**§9.9.4 的變異 (a)(b) 印出的是 `L720-767`，多一行是因為那兩個變異各在該區塊植入一行**，不是兩處數字對不上。**行號會隨本檔任何編輯而漂移，所以它由自檢輸出，不寫進正文其他地方**；本節四組行號全部落在 §9.9.5 之前，因此本節後續的編輯不會使它們失效。

**四個區塊都只被剖析，沒有一個被執行**，包括 §9.9 自身——它不再需要防遞迴的標記，因為不存在遞迴的路徑。§9.7／§9.7b 需要網路與 `gh` 憑證這件事現在只是**寫給人看的註解**，程式不讀它、也不因它而改變任何行為；R4 那一版印的「需 gh 登入」是自檢自己**推斷**的，R6 改成作者登記，**兩者都是在回答一個本輪已經不再問的問題**。
...
- **探針的「執行」完全不在本機制內（R7 收窄後的缺口，明列歸屬）。** 本自檢不執行任何區塊，因此「§9.8 的斷言今天是否仍成立」「#23 §4.4 的 argparse 私有介面是否還在」這類**語意**問題沒有任何自動執行者。處置**不是**把執行加回共用工具，而是拆成兩件不同信任等級的事：
  - **共用的靜態檢查**：可對任意文件（含不受信任者）跑，即本節的自檢。
  - **各卡自己的探針執行**：由該 repo 的 CI 以**具名的獨立步驟**跑**自己**文件裡**指名的**探針（例如「跑 §9.8，斷言退出碼 0」）。這時執行的是自己 repo 內、已經過 review 的碼，與跑任何一支 repo 內測試同一個信任等級；**沒有「對任意輸入執行」這件事，所以不需要隔離邊界**。
  這兩件事都在衍生卡落地，但**必須是兩個步驟、兩份宣稱**，不得再合併回一支「順便幫你跑」的工具——那正是 R4-001。
- 本檔的 §9.8 依此規則取證：**由人直接執行**（`sed -n '<起>,<訖>p' docs/WF_RESOURCE_WRITESET1.md | /usr/bin/python3`，即下限直譯器 3.9.6），輸出與退出碼記在 §9.8 節末。這比 R6 的巢狀執行**更強**——R6 是在 3.14 上跑的，而本輪這次是在宣稱下限上跑的。

#### 9.9.7 在本檔以外的文件上實跑（跨檔一般性取證）

R4 的一般性宣稱是「這支自檢可以驗證**任何**文件的探針可攜性」，而它只在本檔上跑過；R6 以三份文件檢驗它。**R7 收窄之後，那個一般性宣稱本身變小了**——現在宣稱的是「可對**任意文件（含不受信任者）**做靜態可攜性檢查」。這個較小的宣稱有一個大得多的好處：**它不需要對方是善意的才成立**。那正是收窄換來的東西。

**(1) [#23](https://github.com/ruan6047/ai-workflow/issues/23) 的 `docs/WF_EVENT_IDEMPOTENCY1.md`（交付版 `50021ce`，一字未改）。** 該檔不在本卡寫入集內，故以 `git show` 取出到工作區外執行；R7 版自檢、退出碼 `0`：

```text
探針自檢（靜態）：issue23.md；抽出 python 區塊 4 個；執行直譯器 3.14.3；宣稱可攜下限 3.11
執行覆蓋：0——本工具只編譯，不執行任何區塊（R4-001，§9.9.1-D）
可攜門檻：3.9.6（/usr/bin/python3）｜模式 stricter（低於下限：嚴於宣稱而非等價，見 §9.9.1）
區塊數登記：文件登記 4 個
  L256-366：閘門編譯 OK；執行直譯器亦剖析通過
  L533-602：閘門編譯 OK；執行直譯器亦剖析通過
  L752-867：閘門編譯 OK；執行直譯器亦剖析通過
  L958-991：閘門編譯 OK；執行直譯器亦剖析通過
違例 0 筆
[裁決] PASS（靜態：閘門編譯＋區塊數登記；未執行任何區塊）
```

**四個區塊全數通過靜態檢查，沒有一個被執行。** 與 R6 那次（同一份文件的前一個版本、R6 版自檢）差兩點：

- 該檔當時**缺 `probe-blocks` 登記行**而判 FAIL，#23 已於 `50021ce` 補上該行，本次為 PASS。**#23 是本機制的第一個外部採用者，逐檔登記在它身上實測成立**——這一項未受本輪收窄影響，是 R6 留下來仍然有效的部分。
- R6 那次會**實際執行**它的四支探針，R7 不再執行。**這一點對 #23 有實質影響**（它的 §4.4.1 正是靠執行來佐證 argparse 私有介面的可攜性），影響評估與通知義務見 §9.9.9。

**(2) 查核者的利用文件（不受信任的輸入）。** 見 §9.9.4 的 (g)。**它才是 R7 的核心跨檔證據**：R6 的跨檔取證問的是「這支工具在別人的**善意**文件上會不會誤判」，R7 要問的是「它在別人的**惡意**文件上會不會替對方做事」。前者是正確性問題，後者是安全性問題，而**後者無法靠多取樣幾份善意文件來回答**——R6 的取證方法本身有這個盲點，記在 §9.9.9。

**(3) R6 的第三份最小文件（`__main__` 守衛、`sys.argv` 隔離、`sys.exit(0)` 三個約定）本輪移除**，理由與變異 (e)(f) 相同：它釘住的三個約定全部屬於執行路徑，而執行路徑已不存在；留著它就是留一份測已刪機制的測試。
...
**六條裡有三條在別人的文件上不成立，而它們全部寫在本卡、實作也在本卡。** 歸因與 R4 那次相同：不是歸屬遺漏、不是覆蓋率不足，是**宣稱與實作不一致**。差別在量尺又換了一次——R4 學會用「宣稱有多少沒兌現」代替「還要多做多少」，但**只在自己的文件上量**。R6 補的是量的方法：一般性宣稱要用一般性樣本量，故當時的 §9.9.7 有三份文件而不是一份。

> **R6 這段歸因本身還是不夠，第五次因此發生。** R6 加的樣本全是**善意**文件，而 R6 同一輪把預設翻成「執行」——**用「會不會誤判」的量尺去量一個「會不會替對方做事」的問題**，取樣再多也照不到。R4-001 的處置與這個方法論缺陷見 §9.9.9。

#### 9.9.9 R4-001 的處置：為什麼是收窄，而不是再寫一句更強的要求

查核者給了兩條路：**(1) 改成可強制的隔離邊界**（每探針子行程，且限制檔案系統、網路與狀態面），或 **(2) 收窄契約為不執行外部文件並 fail-closed**。並且明文禁止：**不得以作者自我宣告的 `probe-requires`／唯讀承諾作為安全邊界**。本輪走第 2 條。以下是為什麼，包含實測而非推測。

**第 1 條在本卡的寫入集內做不出來，而「做得出一半」比不做更危險。**

- **子行程不是隔離。** 每探針各起一個 `subprocess` 解決的是**狀態污染**（`sys.path`、`os.environ`、已 import 的模組，即 R6 自陳（三）），**完全不限制檔案系統與網路**——查核者那份利用文件放進子行程照樣把檔案寫出來。子行程是必要條件，遠不是充分條件。
- **真正的隔離要 OS 級設施，而它們不可攜。** 本機實測：`/usr/bin/sandbox-exec` 存在，以 `(deny file-write*)` 政策跑一支寫檔的碼確實得到 `PermissionError` 且檔案未生成——**但它是 macOS 專屬、且已被 Apple 標為 deprecated**；同一台機器上 `bwrap`、`firejail` 皆不存在，而衍生卡要掛的 GitHub Actions Linux runner 預設同樣沒有。換言之這條路在**本機能示範、在目標環境不能用**，落地後只會退化成「有沙箱就執行、沒有就跳過」，於是**執行覆蓋隨環境浮動**，而覆蓋浮動正是 R3-001（3.12+ 上靜默 PASS）的形狀。
- **Python 層的假沙箱一律不考慮。** 換掉 `builtins.open`、清 `__builtins__` 之類的做法可被 `os`、`ctypes`、`subprocess` 輕易繞過。寫這種東西並稱之為邊界，就是第五次「宣稱大於證據」。
- **而且它逸出寫入集。** 沙箱政策、canary 驗證、每探針子行程的落地都需要 repo 內腳本與 workflow 檔案，本卡只宣告 `docs/WF_RESOURCE_WRITESET1.md`。**這一條單獨拿出來不足以決定走第 2 條**——若第 1 條是唯一正解，正確反應是停下來回報而不是自己降級；本輪不停下來，是因為上面三點顯示第 1 條在本卡射程內**即使允許改檔也做不出可強制的完整邊界**。

**「再寫一句更強的要求」為什麼不是選項。** R6 已經寫過一句（「採用本自檢的文件其 `python` 區塊必須唯讀，否則要掛登記」），它就在 §13 的自陳（四）裡，而查核者把那句自陳本身變成了利用步驟。**同一個位置再寫一句更用力的話，只會是第五次同根因**：句子沒有執行者，寫幾遍都沒有。本輪刪掉的正是那句話所保護的東西。

##### 對 [#23](https://github.com/ruan6047/ai-workflow/issues/23) 沿用聲明的影響（該卡本輪已 `✅通過`，故逐項列出）

#23 §4.4.1 明文「本卡不定義自己的探針可攜性機制，**沿用 #24 §9.9 作為本 repo 的共同標準**」，其實作卡 A 的驗收欄並列了三件事。收窄後逐件對：

- **編譯閘門（沿用的主體）：不受影響。** #23 的四個區塊在 R7 自檢下全數通過、退出碼 `0`（§9.9.7）。它為此補的 `<!-- probe-blocks: 4 -->` 登記行**仍然有效且仍然必要**。
- **兩項具名適配（`SystemExit(0)` 視為通過、以「需網路／憑證」而非 `import wf_cli` 決定是否執行）：失去標的。** 兩者都只在「自檢會執行探針」的世界裡有意義。**這不是把 #23 的要求打回票，而是那兩項要求所修的缺陷連同機制一起消失了**——它們原本就是在修 R6 的執行路徑。
- **「§4.4 探針須在下限直譯器上實際執行而非只編譯」：不再由共用機制提供，須由 #23 自己的 CI 步驟提供。** 這一項 #23 本來就標明**比本自檢做的多一層**（本自檢當時是在**執行中的**直譯器上執行，不是下限），所以它從來沒有被共用機制滿足過；收窄把「差一層」變成「完全不提供」。**對 #23 是實質變更**：其實作卡 A 需要一個具名的獨立 CI 步驟，以 3.11 執行 §4.4 探針並比對輸出。§9.9.6 已把這個拆法寫成規格；#23 §4.4.1 對此的論證（其探針踩在 argparse 私有介面上，編譯閘門必要而不充分）**本身完全正確且不受影響**，只是實現它的位置從共用腳本移到該卡自己的 CI 步驟。
- **這件事需要 PM 通知 #23。** #23 已 `✅通過`，本卡無權也不應代改它；上述第 2、3 點會使其 §4.4.1 的部分措辭與實作卡 A 的驗收欄與現實脫節。**本卡只做出裁定並列出後果，通知與該卡是否需要 amend 由 PM 判斷。**

##### 這一輪有沒有再立一個沒有執行者的宣稱（照第四次的定義自檢）

逐條列出 R7 新寫下的宣稱，以及各自的執行者：

| R7 寫下的宣稱 | 執行者 | 是不是句子 |
|---|---|---|
| 每個區塊都被 ≤ 3.11 的真實直譯器剖析過 | `subprocess` 實跑，失敗即非 0 | 否 |
| 找不到 ≤ 3.11 直譯器就不出 PASS | §9.9.4 情境 (57a) | 否 |
| 區塊數與文件登記相符 | §9.9.4 (c)(c2)(c3)(c4) | 否 |
| **本工具不執行任何區塊** | §9.9.4 (g)：利用文件實跑，斷言副作用檔案不存在；加上對腳本自身的 AST 結構斷言 | 否 |
| PASS 不代表探針跑得起來 | 裁決行自己印出「未執行任何區塊」 | 否 |

**仍然只是句子、沒有執行者的，有兩處，兩處都寫成了缺口而不是保證**：(i) §9.9.6 要求各卡以獨立 CI 步驟執行自己的探針——那在衍生卡，本卡只能寫規格；(ii) 「3.9 通過 ⇒ 3.11 通過」這個 `stricter` 模式的前提（R4 自陳（一），未變）。**兩者都沒有被寫進任何「已保證」的句子裡**，這是本輪刻意的分寸。

**還有一條不能不說：本節的裁定使本卡的探針執行覆蓋歸零，而這是能力的減少。** 誠實的說法不是「收窄後更安全所以更好」，而是：**本卡原本同時宣稱了兩件事（可攜性檢查、探針仍可跑），其中第二件的實作方式是不安全的，於是本輪把第二件拿掉並指名歸屬**。拿掉之後，「§9.8 今天是否仍 PASS」只由本輪的一次人工執行佐證（§9.8 節末，退出碼 `0`，跑在 3.9.6 上），**不是持續的機械保證**——在衍生卡把它掛上 CI 之前，那是操作紀律。
...
- **CI ＋ 一支 repo 內腳本**：§9.9 的探針自檢——**純靜態**（抽取目標文件全部 `python` 區塊 → 以 ≤ 3.11 的真實直譯器**剖析** ＋ 核對 `probe-blocks` 登記；**不執行任何區塊**），連同 §9.6 第 57 列的六個變異、第 57a 列的無閘門情境、第 57b 列的跨檔一般性、**第 57c 列的不執行邊界**；CI 須釘 3.11 以取得 `exact` 模式（§9.9.6）。腳本落地後即是 **repo 內的共用資產**，[#23](https://github.com/ruan6047/ai-workflow/issues/23) §4.4.1 已裁定沿用它而不另立第二套。
- **各卡自己的探針執行，是與上一項分開的第二個 CI 步驟**（R7，§9.9.6）：由各 repo 以具名步驟執行**自己**文件裡**指名的**探針（本檔＝§9.8；#23＝其 §4.4，且須以 3.11 執行而非只編譯）。**不得再併回共用工具**——共用工具吃的是任意輸入，對任意輸入執行就是 R4-001。對 #23 沿用聲明的影響見 §9.9.9。
- **本卡不落地上述任何一項**——本卡只宣告 `docs/WF_RESOURCE_WRITESET1.md`，新增腳本或 workflow 會逸出自己的寫入集。
...
7. **CLI 路徑引數的正規化**（`--worktree` 等）。§3.1 的封閉 namespace 只規範卡面 `file:` 資源宣告；兩者定義域不相容（此結論 [#23](https://github.com/ruan6047/ai-workflow/issues/23) §10 的 A3 同向認定），理由見該節的界線告示。**且此項目前無人擁有**：#23 已於 `50021ce` §4.1b／§10 裁定其六個承接動詞沒有任何引數需要檔案系統路徑正規化，因而不定義也不引用任何正規化器。**需要者不得逕行引用本節或 #23 當作實作依據**，須先照 §3.1 界線告示的判準追碼，仍需要時連同該節列的 (a)(b)(c) 三項舉證另行開卡。
...
9. **在共用自檢裡執行目標文件的探針，以及為此建立沙箱隔離**（R7 裁定，R4-001）。本自檢**不執行任何區塊**，也不提供任何開關；兩條 disposition 的取捨、以及 `sandbox-exec`／`bwrap` 的可攜性實測見 §9.9.9。**探針的執行不是被放棄，是被移到別處**：各卡自己的 CI 以具名步驟執行自己文件裡指名的探針（§9.9.6、§10）。**要在共用工具裡恢復執行，必須先有可強制的隔離邊界**——那需要 repo 內腳本與 workflow、逸出本卡寫入集，須另開卡並以「這個邊界擋得住什麼、擋不住什麼」為驗收；**不得以作者自我宣告的唯讀承諾充當邊界**。
...
- §1.1、§1.2、§3.2、§3.3、§4.1、§4.2、§8.2、§8.3、§8.7、§8.8、§9.7、§9.7b、§9.8 的所有數字，均由探查程式對真實 repo 與真實 Project #4 產生，非人工清點。四支探針（§9.7 活卡三規則、§9.7b 封閉母體普查、§9.8 離線窮舉、§9.9 探針自檢）**全部內嵌於本檔並可原樣抽出執行**，其中 §9.8 與 §9.9 不需網路與 `gh` 登入。**「可原樣抽出執行」這件事，機械保證的只有『抽出來能被下限直譯器剖析』那一半**（§9.9 的自檢，R7 起純靜態）；**另一半「跑起來斷言仍成立」由人執行取證**——本輪對 §9.8 跑在 3.9.6 上、退出碼 `0`（§9.8 節末），§9.7／§9.7b 需 `gh` 憑證故未於本輪重跑。把後者變成持續保證歸衍生卡的獨立 CI 步驟（§9.9.6）。
...
- **R7（本輪）處理 R4-001（major，blocking）：把 §9.9 自檢的契約收窄成「只編譯，不執行」。** §9.9.1-D／§13 只用**文字**要求採用本自檢的文件其 `python` 區塊唯讀，程式卻對所有未標 `probe-requires` 的區塊直接執行；查核者以三行文件重現：印出「已執行……[裁決] PASS」的同時把檔案寫了出來。本輪移除自檢**全部**執行路徑（`exec`／`compile(..., "exec")` 皆刪除，區塊原始碼只經 `ast.parse` 剖析），撤除 `probe-self`／`probe-requires` 兩個區塊層標記（§9.7／§9.7b 的登記行改寫為純散文註解，兩支程式的**可執行敘述一字未改**），並**不提供任何「執行」開關**。連帶：§9.9.1 由七件改為五件（R6 的 D／F／G 隨執行移除，歷史記在 §9.9.1-D 末段）、§9.6 第 57 列的變異 (e)(f) 移除、原 §9.9.7 (2) 的第三份最小文件移除、新增 §9.6 第 57c 列（不執行邊界的執行者：利用文件實跑＋腳本自身 AST 結構斷言）與 §9.9.9（處置論證、對 #23 的影響、本輪宣稱的執行者清單）、§12 第 9 項。§9.9.3／§9.9.5／§9.9.7 三處輸出**全部以本輪的自檢重跑取代**，§9.8 的輸出改由人直接在 3.9.6 上執行取證。**§1–§8、§9.1–§9.5、§11 一字未改。**
- **R7 的自我歸因：這是同根因第五次，而發作點是 R6 的修法本身。** R6 把預設從「跳過」翻成「執行」，並在 §13 自陳（四）親手寫下「那層意外的安全網沒了、所以文件必須是唯讀的」——**我看見了缺口，然後用一句沒有執行者的話把它填上**。這與 R4 那次「看見了洞卻記成涵蓋率待擴充」是同一個動作，量尺又換了一次仍然不夠：R4 學會量「宣稱有多少沒兌現」，R6 學會「用一般性樣本量」，但**R6 取的樣本全是善意文件**——正確性的量尺照不出安全性的洞。這一輪學到的是第三把量尺：**當一個機制會對別人的輸入做事，就得用『對方是惡意的』來量它**，而不是用「對方會照我寫的規矩來」。另有一件必須說在前面的事：**本輪選擇收窄而非建隔離邊界，不能只用「逸出寫入集」當理由**——若隔離是唯一正解，正確反應是停下來回報。實際理由是隔離在本卡射程內做不出**可強制且可攜**的版本（`sandbox-exec` 為 macOS 專屬且已 deprecated、Linux runner 無 `bwrap`、Python 層假沙箱可繞過），論證與實測見 §9.9.9。
- **R7 對已通過卡的影響（需 PM 判斷是否通知）**：[#23](https://github.com/ruan6047/ai-workflow/issues/23) §4.4.1 明文沿用本節作為共同標準，且該卡本輪已 `✅通過`。收窄後其**編譯閘門與 `probe-blocks` 登記不受影響**（#23 `50021ce` 在本輪自檢下退出碼 `0`，§9.9.7），但其兩項具名適配失去標的、「§4.4 探針須在下限直譯器上實際執行」不再由共用機制提供。**本卡不代改該卡**，逐項後果列於 §9.9.9。
...
- **R6 自陳（一）〔R7 已作廢：自檢不再執行任何區塊〕：探針的「執行」仍在執行中的直譯器上，不在閘門上。** 閘門只負責編譯。#23 §4.4.1 對其實作卡 A 提的是更強的「在下限直譯器上**執行**」，本自檢不宣稱做到。不做的理由寫在 §9.9.1 末段（`stricter` 模式下在 3.9.6 執行會產生與可攜性無關的標準庫假失敗），要兩者等價得先有 `exact`（CI 釘 3.11，§9.9.6）。**這一條是刻意的射程界線，不是遺漏**，但它確實使「探針可原樣執行」只在**編譯**這一半上機械化。**R7 註**：收窄後連那一半的執行都不存在，本條所描述的權衡連同機制一併移除；「編譯這一半」現在就是全部射程，見 §9.9.1-D。
- **R6 自陳，未修（二）：`probe-blocks` 登記與探針本體在同一份檔案裡，同一次編輯可以一起改掉。** 這個不變式擋得住「刪區塊忘了改登記」，擋不住「刪區塊順手改登記」——後者要靠 code review。它與 R4 那個寫死的 `4` 相比是嚴格變好（那個在別人的文件上根本不會響），但**不是密封的**。**這一條 R7 仍然成立且未修**——`probe-blocks` 是本輪唯一保留的、依賴文件作者配合的機制。它與 R4-001 的關鍵差別是**失敗方向**：登記被改錯只會讓自檢**少報一個區塊沒被檢查**，不會讓自檢**替作者做事**；前者是覆蓋損失（要靠 code review 補），後者是任意碼執行。R6 原本寫在這裡的第二句（`probe-requires` 是自我宣告，貼上就會被跳過）**已隨該標記撤除而作廢**——查核者正是拿那句自我宣告的性質建了 R4-001 的利用。
- **R6 自陳（三）〔R7 已作廢〕：執行探針會污染自檢自己的行程狀態。** `sys.path`、`os.environ`、已 import 的模組在探針之間**不隔離**（#23 的 §4.4 探針就會 `sys.path.insert(0, "cli/src")`），因此區塊的執行順序理論上可影響結果。**R7 註**：不執行即無污染。但這一條留在紙上有用——它記錄了 R6 當時**已經知道**需要子行程隔離，卻仍讓執行成為預設；R4-001 打的就是這個間隙。
- **R6 自陳（四）〔R7 已作廢，且它正是 R4-001 的利用說明書〕：把預設從「不執行」翻成「執行」，同時翻掉的還有一層意外的安全網。** R6 當時寫下的代價是：**採用本自檢的文件，其 `python` 圍籬區塊必須是唯讀的，否則要掛 `probe-requires` 登記**。**R7 註：那句話沒有執行者，而查核者照著它建了利用**——一份宣稱唯讀、實際寫檔的文件會被照跑，且裁決是 PASS（重現與現況對照見 §9.9.4 的 (g)）。本輪的處置不是把那句話寫得更強，是**讓它保護的東西不存在**：自檢不執行任何區塊，`probe-requires` 這個標記一併撤除，因為查核者明文禁止拿作者的自我宣告當安全邊界。**這一條是本卡「同根因第五次」的發作點，完整歸因見 §9.9.9。**
- **R6 自陳（五）〔R7 已作廢〕：探針的工作目錄由呼叫者決定，自檢不設定也不檢查。** #23 的 §4.4／§7.1 都以 `cli/src/...` 這類相對路徑讀檔，必須從 repo root 執行。**R7 註**：不執行即與工作目錄無關；該問題隨探針執行一起移交各卡自己的 CI 步驟（§9.9.6），由知道自己該從哪裡跑的那張卡決定。
