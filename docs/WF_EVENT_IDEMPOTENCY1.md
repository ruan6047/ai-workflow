...
canonical `AI_WORKFLOW.md` §4.1 的 lifecycle event 最小 schema **已經**含 `event_id` 與 `state_version`，並已明定「同一卡的 `state_version` 必須單調遞增」。
...
**紀律不是機制。** 任兩個 resume 或人工作業即可撞號，而撞號的後果是該卡永久降級。靠「請勿同時操作」防護等於沒有防護——canonical `AI_WORKFLOW.md` §4.1（「請勿同時操作」皆不構成鎖）已明文把這句話排除在鎖的定義之外。
...
**鎖的鍵必須是 `(owner, project, card_id)` 三元組**，不是 `card_id` 單獨。卡 ID 在跨 repo 的 user-level Project 聚合下不保證唯一（canonical `AI_WORKFLOW.md` §4.3「卡狀態＝Issue」那條明示 user-level Project 跨 repo 聚合即多專案面板），單以 `card_id` 為鍵會讓兩張不同專案的同名卡互相阻塞。
...
> **存活性 metadata 為何不是 canonical `AI_WORKFLOW.md` §4.1「tmux 僅為可選 local adapter」與 §4.3「不得改用聊天、本機檔案或記憶暫代」禁止的本機狀態**：判準是它能回答什麼問題。這些欄位只能回答「這個鎖的持有者還活著嗎」，**不能也不得**被任何路徑用來回答「我上次做了什麼」「我上次寫到哪」。`event_id`、`state_version`、動詞、參數、進度一律不得寫入鎖目錄；resume（§5.1）也不得讀鎖目錄的任何內容作為輸入。違反這條界線，鎖就變成 #16 §4.1 拒絕的預寫意圖日誌 [WAL]。實作卡須以一條檢查釘住：鎖目錄的欄位集合是封閉白名單，新增欄位即失敗。
...
要區分兩者，就必須記住上一次做過什麼——而那正是 canonical `AI_WORKFLOW.md` §4.1「tmux 僅為可選 local adapter」與 §4.3「不得改用聊天、本機檔案或記憶暫代」封掉、且 #16 §4.1 據以拒絕本機預寫意圖日誌 [WAL] 的本機狀態。**任何宣稱能同時解決兩者的機制，都是把本機狀態藏在別的名字底下。**
...
不排序而想別的辦法補救，兩條路都是封的：把 `event_id` 塞進 Project 欄位，等於新增 canonical `AI_WORKFLOW.md` §4.1「lifecycle event 最小 schema」之外的欄位（§11 明列不做）；在本機記住「我已經寫了哪些欄位」，就是 canonical `AI_WORKFLOW.md` §4.1「tmux 僅為可選 local adapter」與 §4.3「不得改用聊天、本機檔案或記憶暫代」封掉的意圖日誌（§6）。**排序是唯一不牴觸既有紅線的解**，而它便宜——三個動詞各動一次寫入順序。
...
| canonical 條文 | 本設計的關係 |
|---|---|
| `AI_WORKFLOW.md` §4.1「lifecycle event 最小 schema」含 `event_id`、`state_version`，同卡 `state_version` 單調遞增 | **沿用，未改 schema**。本檔只補取號程序（§1）、異常處置（§1.2）與 `event_id` 的導出方式（§3）。欄位集合不變、欄位名不變 |
| `AI_WORKFLOW.md` §4.1「`occurred_at` 必須取自寫入當下的系統時鐘」 | **不牴觸**。冪等鍵不取時鐘（§3.3），`occurred_at` 照原規則獨立寫入；排序用 `state_version` 而非時戳 |
| `AI_WORKFLOW.md` §4.1「本機可採原子目錄鎖」；跨主機須具併發控制的服務 | **照該界線分層**（§2），且明示跨主機不可預防、只能偵測。§2.2 的兩個鎖層級仍都是本機目錄鎖，未引入新的協調服務。未宣稱超出 canonical 授權的保證 |
| `AI_WORKFLOW.md` §4.1「tmux 僅為可選 local adapter」／§4.3「不得改用聊天、本機檔案或記憶暫代」 | **遵守**。鎖不持有 lifecycle 狀態；§2.3 的存活性 metadata 有明文劃界（只能回答「持有者是否存活」，resume 不得讀它），並以封閉白名單防止它長成意圖日誌。resume 純讀 GitHub（§5.1）。#16 §4.1 的 WAL 拒絕理由本檔不重述 |
...
- **不承接 `open` 的冪等性**（§8）。
- **不改 lifecycle event 的欄位集合**：schema 是 canonical `AI_WORKFLOW.md` §4.1「lifecycle event 最小 schema」的，本卡只定義既有欄位怎麼取值。
...
- **`L_project` 只保護同機**：與 `L_card` 同一條 canonical 界線（`AI_WORKFLOW.md` §4.1「本機可採原子目錄鎖」），跨主機的兩個程序仍可能建出同名重複欄位，只能事後偵測。
