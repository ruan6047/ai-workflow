...
>
> **四支內嵌探針**（§4.4、§4.5、§7.1、§7.1.2）皆為唯讀靜態分析，不連 GitHub、不寫任何狀態，可自 repo root 重跑。**本檔不為探針釘選單一執行指令**：驗證／閘門角色的執行歸 §4.4.1 所述機制與實作卡 A，人工重跑角色由重跑者自負直譯器選擇。裁定、查證依據與今日實測見 §4.4.2。〔2026-08-12 更正（`WF-EVENT-IDEMPOTENCY1-PROBE-CMD-DRIFT1`／[#52](https://github.com/ruan6047/ai-workflow/issues/52)）：本句原文為「**執行指令一律釘為 `uv run python`**（專案 venv，實測 3.12.13）……但**釘選的仍是 `uv run python`**」，與同檔 §4.4 節末「本輪撤除該處置」**直接相反**。該矛盾在本卡（[#23](https://github.com/ruan6047/ai-workflow/issues/23)）交付當下即已存在，**不是** [#24](https://github.com/ruan6047/ai-workflow/issues/24) 撤除自檢執行能力所造成；原文的完整內容與其逐項失效理由記於 §4.4.2，不另存他處。〕
...
用法：<直譯器> probe44.py [base|inject]；cwd 須為 repo root，直譯器選擇見 §4.4.2
...
**但這張表不是可攜性的保證，它是三個資料點。** 前一版把指令釘選在 `uv run python`（3.12.13）並以「只報實測範圍」收尾——本輪撤除該處置，理由與改法見 §4.4.1。〔2026-08-12 更正（#52）：本句寫於 #23 交付時，是全檔對「是否釘選執行指令」兩個相反指示中**正確的一側**——但當時**檔頭摘要並未同步撤除**，矛盾自交付當下即存在。另外本句只說「撤除」而未說改成什麼，而 #24 `3e45646` 撤除共用自檢的執行路徑後，「該釘什麼」的答案本身也變了。完整裁定見 §4.4.2。〕
...
> **後續更正（2026-08-12，`WF-EVENT-IDEMPOTENCY1-ADAPT-STALE1`／[#41](https://github.com/ruan6047/ai-workflow/issues/41)）——先讀這格，再讀本節其餘部分。這格不是原文，是本卡交付之後補上的。**
>
> 本節其餘文字寫於本卡交付時（`50021ce`），當時 [#24](https://github.com/ruan6047/ai-workflow/issues/24) §9.9 的共用自檢**會執行**目標文件的探針區塊。**#24 後於 `3e45646`（R7）為關閉其查核 finding R4-001（文件宣稱唯讀、程式卻實際執行外部文件）撤除了整條執行路徑**，並一併撤除 `# probe-self`／`# probe-requires:` 兩個區塊層標記。對本節逐項對照：
>
> - **仍然有效（沿用的主體，未受影響）**：以「版本 ≤ 宣稱下限的**真實直譯器**實際編譯」當可攜性閘門、找不到該直譯器即 fail-closed、以文件層 `<!-- probe-blocks: N -->` **逐檔登記**區塊數且缺登記 fail-closed。本節下文那筆登記行**仍然必要且仍然有效**；下文指名 `len(probes) < 4` 是恆真檢查的那一項，是三項要求裡唯一存活到今天的。
> - **已失去標的（下文編號 1、2 兩項具名適配）**：「`SystemExit` 碼 0 視為通過」與「以『需網路／憑證』而非『import `wf_cli`』決定是否執行」。共用自檢不再執行任何區塊，就不存在裁決約定與跳過判準可談。**這兩項不是被駁回，是它們所修的缺陷連同機制一起消失了**——它們原本修的就是那條執行路徑。歸屬裁定見 #24 §9.9.9。
> - **移轉而非消失（下文「§4.4 探針須以下限直譯器執行而非只編譯」）**：這項要求**本身完全成立且未被推翻**，其論證（本檔探針踩在 argparse 私有介面上，語意風險編譯閘門看不見）在 #24 §9.9.1 節末與 §9.9.9 均獲明文確認。但它**不再由共用機制提供**，須由實作卡 A 以一個**與共用靜態檢查分離的、具名的獨立 CI 步驟**、以 3.11 執行 §4.4 探針並比對輸出來滿足（拆法規格見 #24 §9.9.6）。§12 實作卡 A 的驗收欄已附同日期的更正註。
>
> **下文（實跑 A、實跑 B、兩項編號發現、末尾的裁定框、輸出內容）一律保留原樣，一字未刪**，因為它們記錄的是當時真實發生的事：本卡把 #24 的自檢**原樣未改一字**指向自己的檔案實跑，**一跑就踩中兩個缺陷**，#24 再修時又挖出三個。那段歷史的價值不因機制被撤除而減少。本卡**不追溯改寫任何裁決或事件留痕**，只在各失效處加上指回本格的日期標記，並於節末補一次以 `3e45646` 版自檢重跑的**現況**輸出（實跑 C）。
...
>
> 〔2026-08-12 更正（#41）：**裁定本身仍成立**，沿用的主體（下限直譯器編譯閘門＋逐檔登記）未受影響；但**上述兩項具名適配已隨 #24 `3e45646` 撤除共用自檢的執行路徑而失去標的**，§4.4 探針的執行改由實作卡 A 自己的具名獨立 CI 步驟提供。見本節開頭的更正格。〕
>
...
〔2026-08-12 更正（#41）：本段所述的 `75555ca` 版**已被 `3e45646`（R7）取代**。該輪撤除共用自檢的全部執行路徑，故本段所述「已修畢」的兩項具名適配、以及 `sys.argv` 隔離與 `__name__` 設定這兩項執行環境修正，**在今天的共用自檢裡都不存在了**（#24 §9.9.1-D 末段列出了這批一併撤除的項目）。**唯一存活的是下一段的逐檔登記**。這不影響本段記錄的歷史：那些缺陷當時確實存在、確實被修過。見本節開頭的更正格。〕
...
**實跑 B（歷史，同一支自檢的 `75555ca` 版指向本檔）**〔2026-08-12 更正（#41）：**本行原記為「現況」**。`75555ca` 版已被 `3e45646` 取代，下列輸出與實跑 A 一樣**今日不可重現**——每一行「已執行」在今天的自檢下都不會出現。**輸出內容一字未改**，只改了這個時態標籤並補上下方的實跑 C。〕：
...
**實跑 C（現況，2026-08-12，`3e45646` 版自檢指向本檔）**〔本段為 #41 新增；上方實跑 A／B 皆已不可重現，本段是今日唯一可重跑的一次〕。取材與重現方式：自 `origin/main` 的 `docs/WF_RESOURCE_WRITESET1.md` 抽出 §9.9 的 python 區塊存成腳本，以 `/usr/bin/python3`（3.9.6）執行，引數為本檔路徑：

```text
探針自檢（靜態）：docs/WF_EVENT_IDEMPOTENCY1.md；抽出 python 區塊 4 個；執行直譯器 3.9.6；宣稱可攜下限 3.11
執行覆蓋：0——本工具只編譯，不執行任何區塊（R4-001，§9.9.1-D）
可攜門檻：3.9.6（/usr/bin/python3）｜模式 stricter（低於下限：嚴於宣稱而非等價，見 §9.9.1）
區塊數登記：文件登記 4 個
  L256-366：閘門編譯 OK；執行直譯器亦剖析通過
  L564-633：閘門編譯 OK；執行直譯器亦剖析通過
  L783-898：閘門編譯 OK；執行直譯器亦剖析通過
  L989-1022：閘門編譯 OK；執行直譯器亦剖析通過
違例 0 筆
[裁決] PASS（靜態：閘門編譯＋區塊數登記；未執行任何區塊）
```

（退出碼 `0`。與實跑 B 逐行對照，差別**全部在自檢那側，本檔四支探針一字未改**：多出「執行覆蓋：0」一行、四個區塊由「已執行」變成「執行直譯器亦剖析通過」、「實際執行 N 個離線探針」一行消失、裁決行加註「未執行任何區塊」。行號與實跑 B 不同是本節被本次更正加長所致——正如實跑 A 的說明所述，行號由自檢輸出而非人維護。**四個區塊仍全數通過閘門編譯，`probe-blocks` 登記仍相符**，故本節開頭更正格所稱「沿用的主體未受影響」在本檔上是實測而非推論。至於這四支探針今天還跑不跑得起來、斷言是否仍成立，本輸出**不予保證**——那半邊已明確移出共用機制，歸實作卡 A 的獨立 CI 步驟。）

#### 4.4.2 執行指令：依角色分流，本檔不再釘選單一命令

> **本節為 2026-08-12 新增（`WF-EVENT-IDEMPOTENCY1-PROBE-CMD-DRIFT1`／[#52](https://github.com/ruan6047/ai-workflow/issues/52)），不是原文。** 它處置的是一條**在 #23 交付當下即自相矛盾**的指示：檔頭摘要寫「執行指令一律釘為 `uv run python`」，§4.4 節末寫「本輪撤除該處置」。[#41](https://github.com/ruan6047/ai-workflow/issues/41) 的執行者窮舉時發現並劃界，判定修它須決定「現在該釘什麼」、屬實質規格判斷而未動；跨家族查核者同意並列為非阻擋 finding（`attribution: planner`，`root_cause_id: preexisting-authoritative-instruction-drift`）。本節是那筆 finding 的處置，**以本卡 commit 為時點，不追溯改寫任何裁決或事件留痕**。

**先裁定現況，再改文字。** 「該釘什麼指令」預設了「有東西會被執行」。#24 於 `3e45646` 撤除共用自檢的執行路徑後，這個前提必須重新查證，不能沿用——否則只會產生第三個版本的錯誤。查證以 `origin/main` 現況為據，結果三項：

- **共用自檢不執行任何探針區塊。** 自 `origin/main` 的 `docs/WF_RESOURCE_WRITESET1.md` 抽出 §9.9 唯一的 python 區塊逐行檢視並以 AST 列舉全部呼叫點：執行原語只有兩處 `subprocess.run`，兩處送出的都是**常數字面程式**（一支印直譯器版本，一支 `ast.parse(sys.stdin.read())`），探針原始碼一律以 **stdin 資料**傳入被剖析，從不求值；無 `exec`／`eval`／`compile(…, "exec")`／`runpy`／`importlib`／`__import__`，例外處理器只有 `SyntaxError` 與 `OSError`（無 `except BaseException`）。該工具開頭亦自述「本工具不執行任何區塊」。
- **本 repo 沒有任何自動化在執行它們。** 基線不存在 `.github/workflows/`；`scripts/` 只有一支與探針無關的腳本；`docs/` 以外全 repo 無任何處引用本檔探針。
- **唯一存活的執行要求尚未存在，且它要釘的不是 `uv run python`。** §4.4.1 已裁定「§4.4 探針須以下限直譯器**執行**而非只編譯」移轉給實作卡 A 的獨立 CI 步驟，而 §12 A 的更正註明文要求**釘 3.11**；該卡仍待 [#21](https://github.com/ruan6047/ai-workflow/issues/21) 釋放 `cli/`，尚未落地。本機 `uv run python` 為 3.12.13，**高於下限**，而共用自檢挑閘門的過濾條件是「版本 ≤ 3.11」——`uv run python` 在該機制下**根本不具閘門資格**。

> **裁定一：對「驗證／閘門」角色，釘選 `uv run python` 不只是已被撤除，而是與現行機制不相容。** 本檔不為該角色指定任何指令，一律遵 §4.4.1 與實作卡 A。
>
> **裁定二：對「人工重跑」角色，本檔不再釘選單一指令。** 探針「可自 repo root 重跑」的承諾仍然成立、也仍然是探針內嵌而非斷言的理由，但釘選買不到它原本要買的東西（見下）。改為：重跑者自負直譯器選擇；**§4.4 須以 repo root 為 cwd**（它 `sys.path.insert(0, "cli/src")`），§4.5／§7.1／§7.1.2 只用標準庫、不需專案 venv。

**為什麼人工重跑角色也不釘。** 原釘選的理由是「`python3` 在本機解析到三個直譯器，其中一個會直接 `TypeError`」；那筆觀察針對的是**前一版的探針文字**，該文字其後已改寫。以今日四支探針實測三個直譯器（3.9.6 `/usr/bin/python3`、3.12.13 `uv run python`、3.14.3 `mise`）：§4.4 的 `base` 輸出在去掉自報版本行後**三者逐位元組相同**，§4.5／§7.1／§7.1.2 三支皆退出碼 0。**釘選要防的分歧在今天的探針上不復現**，而可攜性宣稱的機械執行者已明確歸屬（實作卡 A 的 3.11 步驟），不必靠讀者手打哪個指令來承擔。**這三個資料點一樣不是可攜性保證**——限定詞與 §4.4 末表相同：它證明的是「今天釘選買不到東西」，不是「任何直譯器都可以」。

> **重跑現況，一併記錄（本卡不修，且與上述裁定無關）**：今日以同三個直譯器重跑 §4.4，三者**一致地**未能重現 §4.4 正文所記的輸出——基線 `cli/` 的參數面已自 106 增為 117，其中 `open --exec-capability-reason`、`open --review-capability-reason`、`assign --capability-deviation-reason`、`amend --ruling-url` 四個旗標未分類，退出碼 1。這正是 §4.4 分類器**設計上應有的行為**（新增參數而未能分類即紅），但它意謂「重跑可重現正文輸出」在今天不成立。修它須為那四個旗標判定型別、動到 §4.1 的型別表，屬實質規格判斷且**超出本卡的單檔寫入集與射程**，故本節只記錄事實並指名，另開卡處理。之所以與裁定無關：該失敗在三個直譯器下**完全相同**，因此不是指令釘選能影響的事——反而是「釘選買不到東西」的又一個資料點。

**本卡對探針原始碼的唯一改動，明說**：§4.4 區塊的 docstring 用法行原為 `用法：uv run python probe44.py [base|inject]`，那是本檔第三處、也是唯一**規範性**的釘選（§4.4 末表那列與 §7.1.1 的 `FIELD_SPECS` 核對指令都是**描述性**的歷史紀錄，記載當時怎麼跑的，故一律不動）。該行已改為不指名直譯器並回指本節。除此之外**四支探針一字未改**，區塊數維持 4，`probe-blocks` 登記不變。

**修改後的自檢實跑（`origin/main` `e8a638c` 版共用自檢指向本檔，2026-08-12）**：
...
區塊數登記：文件登記 4 個
  L256-366：閘門編譯 OK；執行直譯器亦剖析通過
  L601-670：閘門編譯 OK；執行直譯器亦剖析通過
  L820-935：閘門編譯 OK；執行直譯器亦剖析通過
  L1026-1059：閘門編譯 OK；執行直譯器亦剖析通過
違例 0 筆
[裁決] PASS（靜態：閘門編譯＋區塊數登記；未執行任何區塊）
```

（退出碼 `0`。行號與 §4.4.1 實跑 C 不同是本節插入所致；一如該節所述，行號由自檢輸出而非人維護。）
...
| 卡 | 內容 | 前置 |
|---|---|---|
| **A** | `event_id` 決定性導出（含 §3.3 的**解析後** `target` 三元組）＋ §4 型別正規化（含 §4.5 嘗試標籤驗證器）＋ §4.4 分類器 CI（`base`＋`inject` 兩模式，四個負向測試一併釘住）＋ **沿用 #24 §9.9 的探針可攜性閘門並套用 §4.4.1 的兩項適配（`SystemExit` 碼 0 視為通過、以「需網路／憑證」而非「import `wf_cli`」決定是否執行），且 §4.4 探針須在下限直譯器上實際執行而非只編譯**〔2026-08-12 更正（#41）：本欄改為 —— **(i)** 沿用該閘門今日的**純靜態**契約（以 ≤ 3.11 的真實直譯器編譯 ＋ `probe-blocks` 逐檔登記；CI 須釘 3.11 以取得 `exact` 模式，#24 §9.9.6），**兩項具名適配已隨 #24 `3e45646` 撤除執行路徑而失去標的，本欄不再要求**；**(ii)**「§4.4 探針須在下限直譯器上實際執行而非只編譯」**要求不變**，但改由本卡一個**與 (i) 分離的具名 CI 步驟**提供，不得併回共用工具（#24 §9.9.6／§9.9.9）。詳見 §4.4.1 開頭的更正格。〕 | #21 釋放 `cli/`；**且回讀契約（§5.1.1 P1–P5）已定案** |
