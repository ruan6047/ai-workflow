...
**跨直譯器實跑**（R2-002）：`python3` 在本機解析到三個不同的直譯器，三者輸出的分類分佈完全一致——

| 直譯器 | 取得方式 | `base` 結果 |
|---|---|---|
| 3.9.6 | `/usr/bin/python3`（macOS 系統） | 總參數 106／未分類 0／豁免依據失效 0，exit=0 |
| 3.12.13 | `uv run python`（專案 venv） | 同上，exit=0 |
| 3.14.3 | `mise` 安裝於 `PATH` 前段 | 同上，exit=0 |

**但這張表不是可攜性的保證，它是三個資料點。** 前一版把指令釘選在 `uv run python`（3.12.13）並以「只報實測範圍」收尾——本輪撤除該處置，理由與改法見 §4.4.1。

實作卡須把本腳本落為 CI 檢查（`base` 與 `inject` 兩個模式都跑，四個負向測試一併釘住）：**新增參數而未能分類、或豁免依據失效，即 CI 紅**，使 §4.2 的兩條規則（型別閉包、鍵外條件）都有機械執行者，而不是文件裡的一句「應該要」。

#### 4.4.1 可攜性判準沿用 #24 §9.9，不在本 repo 立第二套

「只報實測範圍」撐不住，理由不在測了幾個版本，而在**本節支撐的宣稱沒有版本限定詞**：§4.2 的型別閉包與鍵外條件是無條件的全函數宣稱，而它們的唯一證據就是這支探針。宣稱無條件，證據就必須對**任何能跑 `wfcli` 的人**可重跑；而「能跑 `wfcli`」的範圍不是我碰巧測過的三個點，是 `cli/pyproject.toml` 的 `requires-python = ">=3.11"`。**下限是繼承來的，不因為本節沒宣告就不存在。**

[#24](https://github.com/ruan6047/ai-workflow/issues/24) §9.9 對同一個問題已有裁定：**宣稱下限就要以下限驗證**——抽出文件內全部 python 區塊，交給版本 ≤ 下限的**真實直譯器**實際編譯，找不到這種直譯器即 fail-closed；並在 §9.9.2 以機械反例否決 `ast.parse(feature_version=)` 當閘門（該參數**接受** f-string 取值部反斜線）。那條裁定在本卡一樣成立，故本卡**沿用該機制，不另立第二套標準**。前一版把指令釘在**高於下限**的 3.12.13，正是該裁定要擋的形狀：較新的直譯器跑得過，不構成下限跑得過的證據。

**實跑 A（歷史，把 #24 §9.9 的自檢原樣指向本檔，未改一字）**。自檢取 **#24 `75555ca` 修正之前的 R4 版**；下列輸出**今日已不可重現**，保留是因為它是後述兩項發現的證據，不是本檔的現況——現況見本節末的實跑 B：

```text
探針自檢：docs/WF_EVENT_IDEMPOTENCY1.md；抽出 python 區塊 4 個；執行直譯器 3.14.3；宣稱可攜下限 3.11
可攜門檻：3.9.6（/usr/bin/python3）｜模式 stricter（低於下限：嚴於宣稱而非等價，見 §9.9.1）
  L256-366：閘門編譯 OK；需 gh 登入，僅編譯
  L511-580：閘門編譯 OK；執行失敗
  L730-845：閘門編譯 OK；執行失敗
  L936-969：閘門編譯 OK；已執行
實際執行 1 個離線探針；違例 2 筆
  [FAIL] L511-580：執行拋錯：SystemExit: 0
  [FAIL] L730-845：執行拋錯：SystemExit: 0
[裁決] FAIL
```

（上列 `見 §9.9.1` 是自檢原樣印出的字串，指的是 #24 的該節，非本檔。省略了 §4.5／§7.1 在執行時巢狀印出的內容。四個區塊即 §4.4、§4.5、§7.1、§7.1.2；行號由自檢輸出、隨本檔編輯漂移，故不寫死在正文他處。**四者在 3.9.6 閘門下全數編譯通過**；`stricter` 模式「嚴於而非等價」及其唯一未機械化的前提沿用 #24 §9.9.1，不在此重述。）

**這次實跑同時暴露兩處「原樣沿用」不成立，兩處都在自檢那側、不在本檔的探針上**——沿用是對的方向，但不是即插即用，故逐項指名而非含糊帶過：

1. **裁決約定不相容（上列兩筆 `[FAIL]` 的真因）。** #24 的探針以模組層變數 `ok` 表達裁決；本檔的 §4.5／§7.1 以 `raise SystemExit(1 if bad else 0)` 表達，那是 CI 原生的約定。自檢的 `except BaseException` 把**退出碼 0 的 `SystemExit`** 一律當拋錯，於是兩支**自身裁決全過**的探針被判 FAIL——同一份輸出裡就印著 §4.5 的「失敗項 = 0」與 §7.1 的「未登錄 gh 子指令 = 0」。這是自檢的假陽性，不是探針的缺陷。**處置：共用腳本須把 `SystemExit` 且 `code in (0, None)` 視為通過**；不改本檔探針的表達方式——退出碼比模組層變數更難被誤讀，且它是 CI 唯一真正會看的訊號。
2. **「`imports_wf_cli` ⇒ 需 gh 登入 ⇒ 只編譯」這步推論對本檔不成立。** §4.4 的探針確實 `from wf_cli.cli import build_parser`，但它只做 argparse 內省，**不連 GitHub、不需憑證**。自檢因此把本檔最關鍵的一支探針降級成只編譯不執行。實測反證：`/usr/bin/python3`（3.9.6）直接跑 `base` 與 `inject` 兩模式，輸出與本節正文逐字一致，兩次 `exit=0`。**處置：判準應是「是否需要網路／憑證」，不是「是否 import `wf_cli`」**；在共用腳本補上該判準之前，§4.4 的探針須另行以下限直譯器**執行**而不只編譯。

**第 2 點對本卡不是可有可無的細節，因為本卡探針的可攜性風險與 #24 的不同類。** #24 的四支探針只用標準庫，暴露面是**語法**，編譯閘門正好完整覆蓋。§4.4 的探針則踩在 argparse 的**私有介面**上——`parser._actions`、`argparse._SubParsersAction`、`argparse._StoreTrueAction`／`_StoreFalseAction`、`sub.choices`、`act.choices`、`act.type`——這些沒有相容性保證，而它們的行為若變更**不會產生任何語法錯誤**，編譯閘門看不見。**故編譯閘門對本卡是必要而非充分**：§4.4 的可攜性只能靠「在下限直譯器上實際執行並比對輸出」佐證，那正是上面 3.9.6 那次實跑的角色，也是它不能被降級成只編譯的原因。

> **裁定：本卡不定義自己的探針可攜性機制，沿用 #24 §9.9 作為本 repo 的共同標準。** 引用附帶上述兩項具名適配（裁決約定、需憑證的判準），連同 §4.4 探針的 CI 落地一併歸實作卡 A。
>
> **另有一項與本卡無關但會在第三份文件上失效的常數**：#24 §9.9 的 `len(probes) < 4` 是它自己的區塊數，本檔恰好也是 4 **純屬巧合**。共用腳本須把該數改為逐檔登記，否則它在區塊數本來就多於 4 的文件上是恆真檢查——與 §4.4 自己防的「沉默預設格」同一個病。本卡只指名，不代 #24 修。

**以上三項（含兩處具名適配與該常數）已由 #24 於 `75555ca` 修畢**，故上列「處置：…」諸句是**當時開出的要求**，不是待辦。#24 另修了兩項本卡未觀察到的執行環境洩漏，其中一項直接命中本檔：探針執行時 `sys.argv` 未隔離，而 §4.4 的探針正是 `mode = sys.argv[1] if len(sys.argv) > 1 else "base"`，一旦執行就會把**目標文件路徑當成模式名**。它在實跑 A 從未浮現，只因為 §4.4 被上列第 2 點的錯誤推論擋在執行之外——**一個缺陷遮住另一個**。細節不在此重述，見 #24 §9.9.1-D～G。

逐檔登記既已上線，本檔即依 #24 §9.9 的語法登記自己的四個探針區塊（§4.4、§4.5、§7.1、§7.1.2）。**缺登記一律 fail-closed**（#24 §9.9.1-E），故下一行是本檔宣告自己受該不變式保護的唯一憑據；**增刪探針區塊時必須同步改它，且不得因為「看起來只是註解」而刪除**。

<!-- probe-blocks: 4 -->

**實跑 B（現況，同一支自檢的 `75555ca` 版指向本檔）**：
...
區塊數登記：文件登記 4 個
  ...
  L533-602：閘門編譯 OK；已執行（SystemExit 0＝乾淨結束）
  L752-867：閘門編譯 OK；已執行（SystemExit 0＝乾淨結束）
  L958-991：閘門編譯 OK；已執行
實際執行 4 個離線探針；違例 0 筆
[裁決] PASS
```

（省略四支探針執行時巢狀印出的內容，僅保留自檢自己的行；判準與 `stricter` 模式的語意同實跑 A。**四支全部實際執行**，與實跑 A 只執行 1 支的差別全部來自自檢那側的修正，本檔探針一字未改。）
...
#### 7.1.2a `handoff` 的曝險面已經變了：[#25](https://github.com/ruan6047/ai-workflow/issues/25) 把不可逆效果接上了同一個動詞

上面那條裁定成立時，`handoff` 的首寫不合格只有一種後果：**狀態面半寫入**。狀態面的東西可以重寫，所以「E1 不成立」的嚴重度就到那裡為止。**這個前提已經不成立了。**

共同事實（本卡與 #25 共用同一份，兩側各寫各的一半）：`handoff --next-stage release --cleanup` 的效果寫入順序是 `owner` → `交付狀態` → `最後交接` → `iteration` → Issue body Log；本節判該動詞的首寫是 `owner` 欄位（裸欄位、不具載荷），故 E1 不成立。**#25 已把破壞性收尾——移除 worktree、刪本地與遠端分支——接上這個動詞。**

後果具體是：**清理已完成、`owner` 已寫、Log 寫入前崩潰**時，事件流上**沒有任何可辨識該次寫入的記號**。#25 的 resume 是**觀測式**的（重讀當下事實），因此不會重複刪除；危險不在雙重破壞。危險在另一件事：

> **一個不可逆的動作已經發生，而它的記載不存在。** 本卡對半寫入的全部處置（§5.1 的 resume、§7.2 的 E1–E3）都建立在「事件流裡找得到那次寫入」之上；找不到，就只剩「重跑」這一條，而重跑對已被刪除的 worktree 與分支不是重跑，是對著另一個世界執行。

因此對本節裁定的影響，逐項說清楚，不含糊成一句「更嚴重了」：

- **判定本身不變**：E1 對 `handoff` 本來就不成立，一個已經為假的斷言不會更假。
- **後果嚴重度改變，且改變的是類別不是程度**：從「狀態面不一致，可由 resume 或重跑收斂」變成「不可逆效果已生效且無留痕」。前者的復原成本是幾次寫入，後者要靠人從事件流**之外**的證據（worktree 不在了、分支不在了）反推曾經發生過什麼——那正是 §5.1「純讀 GitHub、無本機狀態」要避免的依賴形態。
- **A′ 的位階跟著改變**：§12 把 A′ 記為「E1 的前置」。它現在同時是**破壞性效果可稽核的前置**——body Log 排到最前面之後，Log 行必然先於欄位寫入存在，破壞性動作至少有一個先行的書面錨點。**這使 A′ 不再只是冪等性的前置，而是 `--cleanup` 這條路徑能不能安全存在的前置。**

**一個屬於 #25 而不屬於本卡的未決問題，在此指名而不代答**：A′ 只保證 Log 先於**四個欄位**寫入，並沒有規定 Log 與**清理動作**的相對順序。要讓「不可逆效果必有先行留痕」真的成立，清理必須排在 Log 之後；那是 #25 側的排序決定，本卡不越界裁定，只把這個條件寫在紙上，使兩側對同一件事有同一份認知。

**本節不修 #25，也不宣稱 #25 有缺陷。** #25 的觀測式 resume 在它自己的射程內是對的；本節記錄的是**兩張卡疊起來之後才存在的曝險面**——單看任一側都看不見，這正是它兩輪、兩位查核者都沒被指出的原因。
...
- **`handoff`／`assign`／`amend --tier` 的首寫尚不自描述**：本卡給出裁定與驗收（§7.1.2），但排序本身是 `cli/` 的變更，歸實作卡 A′。**在它落地前，這三個動詞的 E1 不成立**——本檔不宣稱本設計已使全部六個承接動詞冪等，只宣稱設計完備且缺口已被指名。
  **其中 `handoff` 另加註（§7.1.2a）**：[#25](https://github.com/ruan6047/ai-workflow/issues/25) 已把破壞性收尾（移除 worktree、刪本地與遠端分支）接上 `handoff --next-stage release --cleanup`，故該動詞的 E1 缺口**不再只影響狀態面**——「清理已完成、`owner` 已寫、Log 寫入前崩潰」會留下一個**不可逆效果已生效而事件流無留痕**的組合。#25 的觀測式 resume 使它不會重複刪除，但本卡的 resume（§5.1）對它無從補齊。**這使 A′ 從「E1 的前置」升為「`--cleanup` 可稽核的前置」**；Log 與清理動作的相對順序歸 #25 決定，本卡只指名該條件（§7.1.2a）。
...
| 卡 | 內容 | 前置 |
|---|---|---|
| **A** | `event_id` 決定性導出（含 §3.3 的**解析後** `target` 三元組）＋ §4 型別正規化（含 §4.5 嘗試標籤驗證器）＋ §4.4 分類器 CI（`base`＋`inject` 兩模式，四個負向測試一併釘住）＋ **沿用 #24 §9.9 的探針可攜性閘門並套用 §4.4.1 的兩項適配（`SystemExit` 碼 0 視為通過、以「需網路／憑證」而非「import `wf_cli`」決定是否執行），且 §4.4 探針須在下限直譯器上實際執行而非只編譯** | #21 釋放 `cli/`；**且回讀契約（§5.1.1 P1–P5）已定案** |
| **A′** | §7.1.2 的寫入順序調整（`handoff`／`assign`／`amend --tier`）＋ 首寫載荷探針 CI | 可與 A 併行，**是 E1 的前置；對 `handoff` 另是 #25 `--cleanup` 破壞性效果可稽核的前置**（§7.1.2a） |
