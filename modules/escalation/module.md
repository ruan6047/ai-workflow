...
- `escalation_count`＝本 iteration 內 `move --to <同階段>/退回` 的次數，由 `move` 累加；進執行階段（iteration +1）與 升級 → 進行中 時歸零，⛔ 不由人手改。
- 只有實質 REQUEST_CHANGES 才 `move` 到退回；規劃錯誤前提走 R1 不過退回規劃，等待外部條件走阻塞，同 SHA 重複查核不收。
- `escalation_count` 達 `params.escalate_after`（種子 3）時 `move` 印「達升級門檻」；PM 組升級單後 `move --to <同階段>/升級`。
- 升級單由 PM 組（§0 宣告的交接段）：每次退回的 `wf-return` 留言 URL、blocking finding 的 `root_cause_id`、逐字理由、核心痛點原文、四選一各值「若成立會是什麼證據」。
- 需求方以一則 `wf:ruling` 四選一裁定（換人／退回上一階段／停止／退回無效）；「改規格」⛔ 不是合法值，改規格走規劃階段。
...
- PM：計數以 `move` 為準、組升級單、落裁定；⛔ 不裁定、⛔ 不手改 `escalation_count`。需求方：四選一裁定；⛔ 不代組升級單。

## 2 · 注意事項

- F-escalation-01：只有含 executor 歸屬 blocking finding 的 REQUEST_CHANGES 才轉退回；純 governance／coordination／environment finding ⛔ 不轉退回、⛔ 不計數。
- F-escalation-02：達門檻⛔ 不按整數自動升級；升級單組好才 `move` 到升級。
- F-escalation-03：升級單只寫事實與各值證據，⛔ 不含建議、⛔ 不代需求方選值。
- F-escalation-04：升級單的三次退回理由逐字轉錄自 `wf-return`，⛔ 不摘要。
