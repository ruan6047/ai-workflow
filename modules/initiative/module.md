...
- 觸發者（執行者）、評估者（PM）、核可者（需求方）⛔ 不得**全部**為同一實體；⛔ 不推出任兩者必須不同。兩角能否兼任依既有角色與級別規則另判（如 T0／T1 的 PM 兼執行者依 `roles/pm.md` §2），核可者須為另一實體。量詞裁定來源：https://github.com/ruan6047/ai-workflow/issues/311#issuecomment-5638418412 。T4 兩位查核者彼此不同家族是 `core/tiers.md` §1 的獨立規則，⛔ 不由本條新增第三家族要求。
