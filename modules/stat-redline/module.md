...
```yaml wf-module
{
  ...
  "enable_if": {"kind": "field_contains", "field": "tier_basis.sensitive", "value": "statistics"},
  "fact_source": "卡面 JSON",
  "scope": "card",
  "maturity": "manual",
  "adds": {
    ...
    "flags": [],
    "notes": [{"id": "F-stat-redline-01"}, {"id": "F-stat-redline-02"}, {"id": "F-stat-redline-03"}],
    "handoff_sections": [
      "紅線區塊（本卡的窗口與門檻）",
...
- 查核者至少重跑一個保留驗證集。
- 卡的 `stage_plan` 含研究且能力層級為高階型時，查核者用交回的指令跑 ≥3 個不同族角度的對抗性反測（時間外／母體外／洩漏探針／重抽／規則邊界），在裁決的對抗性反測表逐角度寫支持／推翻／未能檢定（§0）；「推翻」的對象＝該角度的紅線或測試命題，⛔ 不是研究結論真值（`modules/research/module.md` §1）；不含研究的卡⛔ 不要求。
- 第 7、9、10 條的結果解讀通則住 `roles/executor.md` F-執行者-08…11；本表列其在紅線卡的具體化版本。
...
| 9 | 母體隨資料新增而變動是正常狀態；母體數字標資料截止時點，⛔ 不設法凍結數字、⛔ 不把漂移讀成錯誤；因資料更新落後或母體增長（而非模型性質）而失敗或翻面的判定記「不具證據等級」，⛔ 不記為模型失敗證據 | 把母體漂移讀成錯誤 |
