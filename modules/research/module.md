      ...
      "add": [
        {
          "from": "研究/待確認",
          "to": "研究/不可判定",
          "condition": "需求方收口留言逐字「夠了：不可判定」＋查核者 APPROVE"
        },
        {
          "from": "研究/不可判定",
          "to": "需求/待辦",
...
- 目標：把「不知道」變成可判定結論或不可判定；交回單加「量測紀錄（可重跑）」「結論」兩段（§0），結論段起首逐字「可判定」或「不可判定」。
- 從需求階段離開後進入；「夠了：可判定」→ 規劃／待辦，`stage_plan` 無規劃則執行／待辦；「夠了：不可判定」→ 研究／不可判定，再由需求方選重述問題或以不可判定結案（§0 轉移）。
