...
EXCLUSIONS: dict[str, str] = {
    "docs/CONTRACT_TOOL_RECONCILE.md": (
        "產生物，非手寫：它是 `python3 scripts/contract_tool_reconcile.py` 的輸出，"
        "該檔自己在「以下是產生輸出」那段警語裡已寫明表中的 `檔案:行` 錨點會隨其他卡"
        "合併而漂移、且漂移本身不算缺口變化（判定變化以 `--check` 的第 3 個方向為準）。"
        "把產生物納入手寫錨點的守衛只會製造假紅：每次別的卡合併都得重跑產生器才能讓"
        "本守衛回綠，而那次重跑並沒有修好任何東西。要改的是產生器的輸出格式，不是這裡。"
    ),
}
