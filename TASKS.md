...
> **一卡一檔**（canonical §6）：本檔＝**Ledger 索引 only**（常駐輕量），**狀態只住此表**；卡片明細一卡一檔於 [`tasks/<卡ID>.md`](tasks/)，按需載入、卡檔不重複狀態。結案卡 `git mv` 進 [`archive/`](archive/)（見 [`archive/TASKS_ARCHIVE.md`](archive/TASKS_ARCHIVE.md)）。
...
|---|---|---|---|---|---|---|---|---|---|
| WF-13 | — | T4 | 補足迭代式治理、變更分級與需求發現 | GPT-5@Codex | `ai/gpt-5@codex/WF-13`／—不適用 | 0 | ✅通過 | —不適用 | 2026-07-16 人審→合併 |

## 依賴註記（相關卡）

> 規劃／大卡分切時據此判定連動範圍（取代逐卡 `[[]]`；結案卡見 archive）。

- WF-13：更新 canonical 與採用範本；無其他活卡依賴。
