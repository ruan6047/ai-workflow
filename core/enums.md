---
name: enums
when: 寫或讀任何封閉值域（階段、狀態、級別、角色、模組 scope／maturity、紅線三軸、能力層級、db_scope）、寫 schema 的 enum、CLI 合成 choices 時讀
...
{"$id": "wf-enums",
 ...
 "module_scope": {"enum": ["project", "card"]},
 "module_maturity": {"enum": ["ready", "manual", "experimental", "unavailable"]},
...
- `module_scope` 與 `module_maturity` 是模組 §0 `scope`／`maturity` 兩個宣告鍵的值域；`scope` 決定啟用真相（`project` 須名列 `.wf/modules.json` 且 `enable_if` 成立，`card` 只看 `enable_if`），`maturity` 決定自動能力（只有 `ready` 貢獻）。兩鍵缺或值域外＝宣告驗證失敗。
