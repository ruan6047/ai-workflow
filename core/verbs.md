...
| `review <card> --file <交回單.json> --role executor\|reviewer` | 本機交回單 JSON | schema 不合法（D3） | 缺段（依卡面 `tier`：T2 以上的 `unverified`、`note_responses`，與依 `role` 的段，`core/handoff.md` §2）；`note_responses` 的 id 未覆蓋 `notes` 清單；交回單欄位不一致（`review_result` 對 `findings`，PM 判） | 一則帶 `json wf-return` 的留言；⛔ 不動狀態 |
