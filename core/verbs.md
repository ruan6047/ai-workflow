...
- 順序＝框架核心 F-（階段檔 §6 全部＋角色檔只取該卡 `owner.role` 那份，`brief --for` 時取該角色）→ 已啟用模組 F- → 專案 P- → 卡面 `notes` 欄 T-；四個來源累加，下游不改寫上游。
- 核心內序＝該階段檔 §6 → 角色檔 §4（檔名字典序）；來源無 frontmatter（專案層、卡面）只印 `[來源: <來源>/<檔>#<節>]`，卡面＝`card/issues/<n>#notes`。
