...
| 裁定 | 需求方的決定，形狀＝一則留言 | 不是裁決 | 批准、核可、sign-off（除 T4 外） |
...
| 紅線 | 至少 T3 的變更域 | 不是「高風險」的泛稱 | 敏感（作為紅線的同義）、高風險 |
...
| SHA 四種 | 被審＝查核者讀到的 commit；來源＝卡面 `source_sha`；合併基底＝派工單的 merge-base；合併＝main 上的 merge commit | 不寫短 SHA | 目標 SHA、版本、HEAD（作為名詞） |
| 注意事項 | 一份編號清單，四個來源合成 | 不是規則正文 | 踩坑清冊、清單（作為注意事項的同義）、層（作為來源） |
...
| 副作用入口 | 派工單列的、改動會外溢的檔或設定 | 不是三軸的影響面 | blast list、影響面（作為副作用入口的同義） |
