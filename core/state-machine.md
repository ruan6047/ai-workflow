...
八階段固定序：需求 → 研究 → 規劃 → 執行 → 審核 → 部署 → 維護 → 結案。研究、部署、維護是卡級模組，唯一啟用條件＝該卡 `stage_plan` 含該階段。規劃可跳過，由級別決定（T0／T1），仍是核心。`stage_plan` 非空時必為此序的子序列、必含需求／執行／審核／結案（D3）；空＝未填，展開時只有需求階段，`move` 印。
...
```json wf-state-machine
{
  ...
  "transitions": [
    ...
    {"from": "*/進行中", "to": "same/待確認", "condition": "交回；執行階段寫 --source-sha"},
    {"from": "*/待確認", "to": "next/待辦", "condition": "⑤ 過；審核階段 --ruling 種類＝wf-return，缺即印"},
    {"from": "last/待確認", "to": "結案/待確認", "condition": "裁定單（結案確認）"},
    {"from": "**/待確認", "to": "same/退回", "condition": "⑤ 不過（R2–R4）；審核階段 wf-return、結案階段 wf-ruling，缺即印"},
...
CI job `reachability` 跑 `.github/scripts/reachability.py`，對每個合法 `stage_plan`（2026-09-05：16 種）斷言兩件：合成表定義集合內每個非終態有出邊且可達完成或停止；完成與停止出邊為空。另印每個案例從 `initial` 正向走不到的節點，⛔ 不擋（2026-09-07 加，`**/<非終態>` 修好前為 384 個）。同 job 另有三個會讓 rc≠0 的來源，2026-09-07 補宣告、⛔ 不是新增：模組 `adds.notes` 與其 §2 條列 id 的對帳；`--selftest` 的負控（`roles/conduct-common.md` §1「附負控輸出」）；`--selftest` 對正式表的兩個正控——R1 退回目標唯一（`stage_plan` 含規劃時只到 `規劃/退回`、缺規劃時只到 `需求/退回`）與 research 的 `不可判定` 進定義集合。`adds.handoff_sections` 的段名對帳住 `core/dispatch.md`。矩陣隨被測物累加：本檔進 repo 時只測無模組；帶 delta 的模組進 repo 時同 PR 加該模組案例。
