...
6. **共用資料先讀後撈**：本包已備妥的共用資料：`<路徑／無>`。受派者**先讀檔**，需要更新才自己撈，並在報告說明為什麼。⚠️ 治的是**額度**不是效能——Projects v2 只有 GraphQL 介面，額度 5000 點／小時且與 REST 分開；單次查詢很便宜（2026-08-18 實測 152 item 的 `project item-list` 為 2 點），燒光額度的是**同一份資料被多個受派者各撈一遍**。額度見底時每個 `wfcli` 動詞都會失敗。
7. **CLI 探索紅線**：不得真跑爬蟲、訓練等有副作用的 CLI。本專案**當前仍有副作用的入口**：`<清單／無>`（來源＝專案 Contract，canonical §7）。
