...
2. **變更分級 + 部署閘門**：依 canonical T0–T4 按風險、範圍、可逆性選閘門；**T2 以上程式碼(A 類)每卡開分支** `ai/<模型或工具>/<卡ID>`；**只有 `main`（已審核合併）能部署，分支不部署**。
...
5. **需求與大型工作**：T3/T4 先完成 Discovery Gate；大型工作以 Initiative 管理 spec 基線、依賴、里程碑與變更，子卡採可驗證切片。
