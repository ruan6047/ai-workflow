...
| 角色 | 責任 |
|---|---|
| 需求方 | 判「該不該做」、升級四選一裁定、**結案階段的 ④**；AI 不可自行派工 |
| 人工查核 | 需求方的**同人異帽**：判「前提對不對」 |
| PM | 流程，以及狀態的**唯一 writer**（每次轉移寫四欄轉移記錄）；⛔ 不判內容 |
| 第二 PM | PM 自產物的檢查（本專案＝Codex）。⚠️ 它有**完整寫入通道**——「只查核、⛔ 不動狀態」是紀律⛔ 不是機制；身分靠 session ID ＋ transcript 核對 |
| 執行者 | 在獲認領的分支／worktree 實作與自測，交回含自評的交付報告 |
| 查核者 | **跨家族**；對照目標與證據驗收，`core_pain_resolved` 具**否決權**；不得代改被審 source branch，但必須留下 finding／結論 |

- **同卡同輪一人一角。**
- **PM 兼執行者：T0／T1 可、T2 以上 ⛔ 不可。** 判準是**風險軸**，⛔ 不得用難度軸決定。

> ⚠️ **本表由七角色換為六角色**（`WF-REDESIGN-W2A`，依決議紀錄
> [`WORKFLOW-REDESIGN-2026-08-30.md`](docs/research/WORKFLOW-REDESIGN-2026-08-30.md) §六，
> 取代清單 §一 row 3）。舊表整塊刪除，⛔ 不留屍體。**裁撤四名逐名處置如下**，
> ⛔ 不再有「一律不得映射」那條籠統禁令——它與 §1.1 的 `Coordinator` 映射句自相矛盾
> （`WF-REDESIGN-W2A` R1-002／AC9，二者只能存一個，取後者）：
> (a) **`Coordinator`**：§1.1 逐字把它的職責指給「唯一 PM 祕書 session」⇒ 即本表的 **PM**。
> 這是**既有 canonical 條文本來就有的映射**，⛔ 不是本卡新造的。
> (b) **`技術規劃者`**：[`stage-rules/planning.md`](stage-rules/planning.md) 的分角色表逐字
> 寫成「執行者（技術規劃者）」⇒ **規劃階段內**即本表的 **執行者**。
> ⛔ **不得把它外推成通用映射**——那張表管的只有規劃階段。
> (c) **`設計者`，以及 Discovery 階段的證據負責人**（該名稱是決議 §二 的污染符，
> ⛔ 本檔不再書寫）：決議 §六 ⛔ 未給映射，本卡也⛔ 不造一個。§3 的三條 Gate 定義已改寫成
> ⛔ 不指名這兩者，⛔ 不得由本表反推它們等於誰。
...
### 0.0 階段與狀態（8 階段 × 10 狀態，轉移 delta 制）

> ⚠️ **本節定義目標狀態，尚未切換；cutover＝切換 Initiative。**（形狀沿 §0.1 先例。）
> ⚠️ 本標記涵蓋 §0.0 的**目標模型**（階段、狀態、轉移 delta）。⛔ **不**涵蓋三處：
> §0 的類型表、下方「變更級別」段，以及本節末尾「切換前的現行看板語彙與仍有效的約束」
> 小節裡標為**現行有效**的句子——那三者今天就在拘束人。
> ⚠️ 刻意逐句界定而⛔ 不用一句話蓋全節：**⛔ 不得以整段「非條文」覆蓋仍具強制力的句子**。

**階段（8）**：`需求 → 研究* → 規劃* → 執行 → 審核 → 部署* → 維護* → 結案`
（`*` 可跳過；跑哪些由開卡表單的「階段計畫」宣告）。

⛔ **「合併」與「收尾」不是階段。** 判準：**階段是有人在那裡做判斷的地方**，兩者都是 CLI 的
機械步驟。需求的固定動作＝**撤銷**（`deleteProjectV2Item`；卡回清單、卡ID 保留、iteration
延續）；結案的固定動作＝**封存**（`archiveProjectV2Item`）。

**狀態（10）**：通用 7＝`待辦`／`進行中`／`待確認`／`完成`／`退回`，加上正交的 `阻塞`／`升級`；
階段專屬 3＝研究的 `不可判定`、維護的 `運行中`、結案的 `停止`。
**專屬狀態的判準（本身即規則）**：換成通用狀態**會不會丟失行為差別**——維護的「失效」不合格
（行為等同 `待辦`），故⛔ 不立為專屬狀態。

`停止` 是結案專屬終態、⛔ 無出口；裁定必含決策／原因／**可證偽的復活條件**，復活＝開新卡。
`阻塞` 是等得到的；解除時回到轉移記錄的 `from`。

**轉移採 delta 制**：`stage_next[階段][狀態] = BASE | ADD & ~REMOVE`，各階段規則檔只寫自己的
delta（見 [`stage-rules/`](stage-rules/)）；可達性檢查跑在**規則檔變更時**。
⚠️ bitmask 是 **CLI 端驗證**，⛔ 不是看板約束。

#### 切換前的現行看板語彙與仍有效的約束

⚠️ **本小節⛔ 不是單一效力等級。** 逐句標明效力與 owner——上一版把整段標成「非本節條文」，
而該段裡有「新寫入不得用」「不得人工改寫」「必填」「不得結案」「不得 release」這類**仍在
拘束人**的句子，一句話蓋過去就是把強制條文寫成敘述（`WF-REDESIGN-W2A` R1-001）。

看板 `交付狀態` 欄的現有值，排列順序為 `💡需求 → 🔬研究中 → 🧭規劃中 → 📥Backlog → 🔨執行中 → 🔍待查核 → ✅通過 → 📦已合併 → 🏁完成`，另有 `↩退回`、`⏸阻塞`、`🚨已升級`、`🛑已停止`。

| # | 句 | 效力 | owner／退場路徑 |
|---|---|---|---|
| a | 上面那條值域與它的排列順序 | **敘述、⛔ 非約束**——§0.2 表七 D1 已逐字裁定「§0 那一行是敘述順序，不是約束」；轉移的**規範身分**已由 §0.0 的 delta 制取代 | 值域現況歸**切換 Initiative**（決議 §一 row 2） |
| b | **廢止的歷史值**（向後相容，已寫的卡留著，新寫入不得用）：`🚧進行中`、`⏳待執行` | **現行有效**（強制） | 切換 Initiative，隨值域一併退場。⚠️ §0.2 表七 D4 逐字記載兩值仍在 `project.FIELD_SPECS` 裡、`--status` 寫得進去 |
| c | 不可覆寫 event log 是狀態歷史，Ledger 是由 event log 產生的 current-state projection；兩者不得各自人工改寫 | **現行有效**（強制）。⛔ 與十五值無關 ⇒ ⛔ 不隨 cutover 退場 | canonical §4.3（唯一寫入通道） |
| d | `🛑已停止` 必填決策與原因後封存 | **現行有效**（強制）。⚠️ §0.2 表七 D3 逐字記載它今天**沒有機械執行者** | 值域字面歸切換 Initiative；語意由 §0.0 的 `停止` 承接（裁定必含決策／原因／可證偽的復活條件） |
| e | 部署狀態獨立：`—不適用`，或 `⏸未部署 → 🚀待部署 → ⏳部署中 → ✅已部署 → 🧪驗證中 → ✅已驗證` | **敘述**（值域現況） | 決議 §一 row 7 → **切換 Initiative**（退位成部署階段產出物） |
| f | 失敗／回滾不得結案 | **現行有效**（強制） | 同 row 7；語意由 §0.0 的結案階段承接 |
| g | release 事件必以**終態**交付狀態落地：免部署卡 release 即 `🏁完成`，需部署卡在部署 `✅已驗證` 前不得 release | **現行有效**（強制）。⚠️ §0.2 表七 D6 逐字記載 `--next-stage release` 不檢查前身狀態 | 值域字面歸切換 Initiative；行為缺口歸 CLI 動詞卡（D6 已登記） |
| h | 結案清單（終態事件、封存、Ledger、資源清理、對帳）見 [`worktree-lifecycle.md`](templates/worktree-lifecycle.md) | **現行有效**（指向範本） | canonical §6 與該範本 |
...
> ⛔ **本節已由 §0.0 取代（superseded）。** 本節保留為**歷史說明**——它記錄兩軸模型第一版
> （7 階段、另一套通用／專屬狀態）的裁定與當時的窮舉證據，⛔ **不再是目標狀態的宣告**
> （`WF-REDESIGN-W2A` AC8）——那句自稱已從本節移除，在本檔僅存 §0.0 節首一處。
> ⚠️ 這裡刻意⛔ 不把那句自稱的原文再抄一次：抄了它就自己變成第二處，
> 而「僅存一處」正是這一行要宣稱的東西。
> (a) 刻意保留而⛔ 不刪。
> (b) 為什麼：下方「本卡條文的執行者狀態」表是 `WF-STAGE-STATE-TWO-AXIS1` 的逐句窮舉稽核，
> 而決議紀錄 §一 row 4 把「§0.1 過渡註記」的移除指給**切換 Initiative** ⇒ 刪掉它等於代那張卡
> 決定，也會毀掉那份窮舉的可證性。
> (c) ⛔ **不得由本節推出任何現行的階段數或狀態值域**——與 §0.0 衝突時一律以 §0.0 為準。
> 已知的三處差異：階段 7 → **8**（新增結案）；`已合併` ⛔ 不再是狀態（合併不是階段，
> 收尾位置由結案階段承接）；「宣告了維護階段的卡永遠不結案」**已被推翻**，維護的出邊見
> [`stage-rules/maintenance.md`](stage-rules/maintenance.md)。

⚠️ 本節有**一條要求⛔ 未被 §0.0 取代、今天仍成立**：**消費端相容層必須先於欄位切換落地**
——`cpbl` 有六個檔綁該語彙，而 `roadmap_lines.gate_of` 對未知狀態 fail closed，其 docstring
逐字「狀態詞彙表變更時應該要有人知道」。承接者見下方執行者狀態表第 1 列。
...
⚠️ **原本在這裡的「結案的觸發」段已搬到
[`stage-rules/closeout.md`](stage-rules/closeout.md)**（`WF-REDESIGN-W2A` AC8），⛔ 原處不留
一份。搬移時**改寫了它的進入子句**：舊文只寫得出一條入邊（純文件卡止於審核），且逐字宣告
「宣告了維護階段的卡永遠不結案」——後者與 [`stage-rules/maintenance.md`](stage-rules/maintenance.md)
的 `運行中 → 結案／完成` 互斥 ⇒ 新居所改以實測三條入邊取代。
...
| # | 條文位置 | 今日執行者 | 邊界外會發生什麼 |
|---|---|---|---|
| 1 | §0.1 消費端相容層須先於欄位切換落地 | ⛔ 無。`cpbl` 六個檔綁舊語彙。**承接者＝切換 Initiative**（決議 §十之二 第 3 點逐字列 cpbl 側語彙相容） | 切換即 `roadmap_lines.gate_of` 對不上，兩專案看板同時失真 |
| 2 | §0.1 外部觸發交付物須宣告維護階段 | ⛔ 無。承接者＝子卡的 `open` 驗證 | 排程掛掉沒有狀態可表達，重演「交付後沒人看」 |
| 3 | §0.1 機械步驟留下的位置須有狀態可表達 | ⛔ 無，本身是設計判準。**承接者＝§0.0**——該節重新裁定「合併與收尾⛔ 不是階段」，收尾位置改由結案階段承接 | 回到單欄位，`cpbl#166` 形狀在看板上仍不可見 |
| 3b | §0.1 不得以「終態佔比高」證成封存 | ⛔ 無。承接者＝查核者人工審 | 用一個只反映專案年齡的比例證成工作，如本卡 R4 前的三輪 |
| 4 | §0.1 結案不可由角色直接設定 | ⚠️ 僅事後偵測：`cleanup.classify_state`。逃生門 `handoff_cmd.py` 的 `if args.status:` 分支敞開。**承接者＝`WF-REDESIGN-W3′`**（需求方 2026-09-01 裁定；清單項 `ruan6047/ai-workflow#221`，開卡時列入射程）。理由：逃生門在 CLI 內部⇒ 屬 W3′ 面，且它是 `if`／`elif` 的**第一個**分支——走它就整個跳過前身狀態閘門，⇒ 靜默繞過、人工審抓不到，故⛔ 不適用「承接者＝查核者人工審」那種弱承接。⚠️ 這是**指向尚未開的卡**，形狀同本表第 1 列（承接者＝切換 Initiative） | 見該節逐字說明 |
...
| 5c | §6.3 每張卡必有簡介 | ⚠️ **通道有、守衛無**：`open --brief`／`amend --brief` 已落地（`WF-CARD-BRIEF-AND-TWO-AXIS-WRITE1`），但 `--brief` 可選且 `validation.py` 不驗。承接者＝`WF-CARD-BRIEF-BACKFILL1`（回填） | 一張沒有簡介的卡 `open` 得出來（`--brief` 可選）；它會不會在 `amend`／`handoff` 被擋下，⛔ 本格只說「**以下述量法搜不到**會擋它的路徑」，⛔ 不說「不存在」。⚠️ **本格刻意不記數字**——⭐ 數字量的是**回填活動**，本欄問的是**邊界外會發生什麼**，兩者正交：回填把每張卡都補齊，`--brief` 仍然是可選的（`bc5bcbb` 寫下的 198/8 兩小時內即成 201/11，正是不記它的理由）。**重新導出的量法（⛔ 非封閉述詞）**：見 §6.3 同段——枚舉 `.brief` 的 importer 與哨兵字面，逐個判它在**缺**簡介時是 fail-open 還是拒絕。⛔ 不以「某個符號有沒有被改」為判準——落點不必在 `validation.py`；`doctor.brief_present` 是事後稽核、`disposition` 為 `migrate`，一旦執行即為假。⚠️⚠️ **它得到的是下界**：字面枚舉**找得到**擋下路徑，⛔ **證不出不存在**（以別的標記自寫檢查、或由資料驅動的規則落點都逃得過）。⇒ 本格的宣稱到此收窄為「**以該量法搜不到**執行者」，⛔ 不是「不存在執行者」——⭐ 前一版在此寫「封閉」而它不成立，同一病灶換個居所而已。本格何時再變假：該量法的輸出裡出現任何一個在缺簡介時回非零 rc 或拋未攔截例外的成員（今日下界為 0） |
...
- **Discovery** 回答「是否在解對的問題」：T3／T4、大卡、跨系統與不可逆變更先完成 Discovery Gate，明列目標使用者／利害關係人、觸發情境、痛點、成功條件、非目標、已知證據與待驗證假設。**執行者**將證據標為使用者研究、既有產品資料、公開研究或 AI 推測；AI 可調查既有資料、程式脈絡與競品，但不得將推測當作使用者證據。需求方確認問題與研究限制後，才可進 Design。
- **Design** 回答「解法是否適合使用者」：所有使用者可見的 T3／T4 卡，及 Initiative 的使用者旅程改變，必須有 Design Brief，定義主要流程、資訊架構、正常／空／錯誤／權限狀態、可及性與可用性驗收。**執行者**提出方案，需求方核可取捨與驗收；真實訪談、prototype 或可用性測試由需求方依風險決定，AI 只能協助準備與整理。純技術 T3／T4 可標註 Design Gate `N/A`，但必須記錄理由。
- **Plan** 回答「如何安全實作」：**執行者**（規劃階段內即技術規劃者，見 [`stage-rules/planning.md`](stage-rules/planning.md)）只在已核可的 Discovery／Design 基線上產出 spec、依賴圖、風險、驗證與子卡切片。發現不可行或成本超出邊界時，回寫受影響的 Discovery／Design brief 並重新核可；不可只在實作卡內改變方向。T0–T2 至少在卡或 commit 說明範圍與驗收。Plan 產出必含建議執行／查核能力層級與理由（層級語彙見專案 `MODEL_ROUTING.md`）；建議反映任務風險，不得因當下額度預先降級——派工可依可用性偏離，偏離與理由記入 claim 事件。
> ⚠️ **上面三條 Gate 的動作者由已裁撤的舊角色名改為「執行者」**（`WF-REDESIGN-W2A` AC9）。
> 依據：八份 [`stage-rules/`](stage-rules/) 的「各角色」表**逐檔**把該階段的產出工作指給執行者。
> (c) ⛔ **不得由此推出「被裁撤的舊名＝執行者」**——這裡改的是**誰做這件事**，
> ⛔ 不是宣告一組映射；舊名的逐名處置見 §1 表下註解。
> ⚠️ ⛔ **§3.1 規劃閘門三級制⛔ 未被退役**：它是**規劃深度政策**（Initiative／T4 的 grilling、
> T3 的核心痛點三問），與 §0「變更級別」段的**最低閘門**是兩回事；§0.0 的「尚未切換」標記
...
且 `validation.py` 完全不驗簡介 ⇒ **沒有簡介的卡寫得出來**；而它會不會在後續動詞被擋下，
⛔ **本節只能說「以下述量法搜不到會擋它的路徑」，不能說「不存在」**——理由見下一段的下界
說明。⚠️ **此處刻意記量法而非比數**——⭐ 比數量的是**回填活動**，本節問的是**執行者
的有無**，兩者正交：回填把每一張卡都補齊，`--brief` 仍然是可選的（本句初稿寫下的 198/8
在兩小時內即成 201/11，正是不記它的理由）。

**要找的東西**：`cli/src/wf_cli/` 內任何**以「卡面缺簡介」為條件**而使動詞回非零 rc 或拋出
未被攔截例外的路徑。⛔ 不以「某個具名符號有沒有被改」為判準——落點不必在
...
**重新導出的量法**（⛔ 刻意只記量法，⛔ 不寫成封閉述詞）：`git grep -n -E "from \.+brief
import|import brief" -- cli/src` 取全部 importer 逐個判處置（⚠️ 點號寫 `\.+` 而非 `\.`——
`open_cmd` 走 `from ..brief import`，只寫一個點會漏掉它，⭐ 本句初稿就是這樣漏的），
再加 `git grep -n -E "card-brief:begin|## 簡介" -- cli/src` 驗哨兵字面有沒有自寫解析。
⚠️⚠️ **這個量法得到的是下界**：它**找得到**擋下路徑，⛔ **證不出不存在**——以別的標記自寫
檢查、或把落點放進資料驅動的規則表，都逃得過字面枚舉。⇒ 本節的宣稱到此**收窄**為
「**以該量法搜不到**執行者」，⛔ 不是「不存在執行者」。⭐ 這一段前一版寫成「證偽述詞
（封閉）」，而封閉性在本 repo **做不到**（欄位寫入就有兩條互不呼叫的原語，見
`project.py` 的階段欄註記）⇒ 硬寫封閉只是把同一個病灶換個居所，**看起來還更像保證**。

今日跑該量法的結果（下界 0，⛔ 非「不存在」）：唯一會因缺簡介拋 `BriefError` 的
`brief.parse_block`，在 `brief.py` 之外**零 importer**，其唯一呼叫點是 `try_parse_block`
自己並 `except BriefError: return None` ⇒ 缺簡介**到不了**任何拒絕路徑；其餘 `.brief`
消費者非 fail-open（`try_parse_block`／`drifted`）即只在**有給**簡介時驗形狀
（`validate_shape`，⛔ 不因缺席觸發）；哨兵字面只落在 `brief.py` 的常數、`card.py` 與
`amend_cmd` 的區段插入與說明文字上，⛔ 無自寫解析。要**簡介覆蓋率**的現值：以
`wf_cli.brief.parse_block` 對 `list_items` 逐張試解析——⚠️ 那是另一個量、量的是**回填活動**，
⛔ 不是本節在問的執行者有無。依 ROADMAP §0 的判準「有機械執行者會擋下它」，本條**未達成**。
