...
交付狀態為 `💡需求 → 📥Backlog → ⏳待執行 → 🔨執行中 → 🔍待查核 → ✅通過 → 📦已合併 → 🏁完成`，或 `↩退回`、`⏸阻塞`、`🚨已升級`、`🛑已停止`。**`📥Backlog` 是「已登記、未排程」，不是佇列位置**——進到它只代表這個發現被登記下來且現在不做，不代表排到隊、也不代表將來會做；要它動起來須有需求方裁定，不會自動前進。⚠️ 預設 Issue 視圖看不出這件事（交付狀態住在看板欄位，不在 Issue 本體），所以**「開著的卡數」不等於「待辦數」**；統計待辦、判斷是否發散或決定要不要收斂前，必須先把它扣掉。不可覆寫 event log 是狀態歷史，Ledger 是由 event log 產生的 current-state projection；兩者不得各自人工改寫（狀態面實作與唯一寫入通道見 §4.3）。`🛑已停止` 必填決策與原因後封存。部署狀態獨立：`—不適用`，或 `⏸未部署 → 🚀待部署 → ⏳部署中 → ✅已部署 → 🧪驗證中 → ✅已驗證`；失敗／回滾不得結案。release 事件必以**終態**交付狀態落地：免部署卡 release 即 `🏁完成`，需部署卡在部署 `✅已驗證` 前不得 release；結案清單（終態事件、封存、Ledger、資源清理、對帳）見 [`worktree-lifecycle.md`](templates/worktree-lifecycle.md)。
...
- **決策 100% 屬需求方本人**：開卡、派工、追加前置、資源調度、結案。AI 不得自行開卡或代擬優先序。需求方不在場時 **AI 只能續做已派工作**，待決事項依 §6.1 條款 1 寫成**報告的一節**交回等裁定——不另立佇列、不建背景待辦、不先斬後奏。
...
11. **一根問題一張卡**：卡的範圍單位是「問題」不是「授權」；一根問題的多個窄寫入授權列在同一張卡。執行者遇授權缺口時**停下**，把「阻塞發現」寫回報告交需求方裁定，不得自行擴權或開新卡（§3.2）。
...
- **T2 快線不另立留痕檔（2026-08-15 廢止 `BUGS.md` 式滾動 log）**：快線的留痕＝分支、先紅後綠的回歸測試、獨立輕量查核，以及 §6 的 commit trailer——**不再要求額外滴一行到獨立檔案**。廢止依據是實測：該出口在本 repo 建檔至今 **0 筆**、在採用專案 **1 筆**，而它與 `ADOPTION.md` 都自稱依據「canonical §3」，本檔卻**從未提及它**——是一個沒有依據、也幾乎沒人走的出口。改掛 commit trailer 的理由是**兩個條件它都占**：trailer 對每個 commit 本來就必填（§6），且有寫好的偵測器可列舉漏寫；滾動 log 兩者皆無。⚠️ **偵測器不是閘門**——trailer 守衛住在 `wfcli doctor`，須有人主動跑，CI 不跑它，它擋不住任何一次 push。這裡換掉的是「兩個都沒有」與「兩個都有一半」，不是「沒守衛」與「有守衛」。既存的 `BUGS.md` **封存唯讀**，不再追加，也不再要求新專案建立。
...
- 執行中發現授權缺口，走的是這條路：**停** → 依 §6.1 條款 1 把「阻塞發現」寫成**報告的一節**回報需求方 → 需求方裁定。裁定「現在做」才擴充現卡授權或開新卡；裁定「不是現在」則**開卡並直接置 `📥Backlog` 登記**——那是登記，不是排程（§0）。執行者不得自行決定，也不得自行開卡。
...
- **單一祕書跨專案服務**：一個 PM 祕書 session 以 repo 為 namespace 操作各專案的狀態面；**待裁事項一律回到同一位需求方**，不因專案不同而分流、不各自累積。
