...
「實務曾三次停在 `📦已合併` 留下假活卡」；2026-08-24 實測**四張卡正停在該值，最久九天**。
⇒ 它不是瞬間通過的點，是真實位置。
...
本卡**只交付條文**。以下六項**刻意不做**，⛔ 不是遺漏：

1. **不實作** wfcli 寫入端、Project 欄位切換、cpbl 讀取端。
2. **不回填既有 187 張卡的簡介**——回填本身要另開卡，且 ⛔ 未驗證簡介對 AI 判斷相關性的實效。
3. **不做「結案時強制指名承接者」**（Discovery 假設 A，已否決）。
4. **不做 root_cause_id 的自動清單產生**（Discovery 假設 C，已否決：322 occurrence 對
   315 個相異名，⇒ 自動歸併會產出幾乎等於原始清單的東西）。
5. **不把「這張卡值不值得做」交給跨家族查核者**——理由見 §6.4 第 2 個來源。
6. **不處理 aiwf#122**（「允許的狀態轉移」下放後兩專案皆未填），也**不處理 Backlog
...
⚠️ 實得 **12 行對應 11 句**（表列 11 行）——第 4、5 行同屬第 4 句（該句因加註今日逃生門而跨兩行）。
...
終態卡以 `archiveProjectV2Item` 移出活卡視圖。2026-08-24 實測 Project 4 共 187 個 item
中 110 個是終態，佔 **59%**。⛔ **維護卡不封存**——那會重演「交付後沒人看」。
