...
⭐ 這直接修掉 2026-08-22 `cpbl#166` 的形狀：`illegal_terminal_before_cleanup` 的本質是
...
**全階段族（2）**：`宣稱超過證據`、`列舉或覆蓋不完整`。
**階段族（11）**：`守衛涵蓋不足或可被繞過`、`交付未落地或未接線`、`身分或歸屬對應錯誤`、
`文件與現實漂移`、`程序或規格照字面不成立`、`可重現性不足`、`狀態轉移或生命週期`、
`留痕失真或遺失`、`解析或正規化錯誤`、`並發或時序不安全`、`資源或寫入集宣告`。
