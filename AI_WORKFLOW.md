...
且 `validation.py` 完全不驗簡介 ⇒ 絕大多數卡沒有簡介區塊。⚠️ **此處刻意不記比數**——
它每天在變（本句初稿寫下的 198/8 在兩小時內即成 201/11），而本節正是在講執行者的有無，
⛔ 不該用一個會腐爛的定值當論據。要現值：以 `wf_cli.brief.parse_block` 對 `list_items`
