...
| 4 | §0.1 結案不可由角色直接設定 | ⚠️ 僅事後偵測：`cleanup.classify_state`。逃生門 `handoff_cmd.py` 的 `if args.status:` 分支敞開。**承接者＝⛔ 尚無具名卡**——收斂 `--status` 屬 CLI 動詞改造，而決議 §十 的 W3′ 清單⛔ 未列此項；§0.2 表七 D4 只寫「已登記」、⛔ 未指名卡號 | 見該節逐字說明 |
