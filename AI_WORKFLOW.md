...
> 取代清單 §一 row 3）。舊表整塊刪除，⛔ 不留屍體。
> (a) **§3 任務流程仍以本表已裁撤的名稱敘述 Gate 流程**，這是刻意留下的、⛔ 不是遺漏。
> (b) 為什麼：取代清單⛔ 沒有「§3 角色名」那一列 ⇒ 它沒有 owner，而⛔ 不在本卡射程。
> (c) ⛔ **不得由本表推出裁撤名與新六角色的對應關係**——決議 §六⛔ 未給映射，
> 自行推一個就是憑空造規則。讀 §3 時該批名稱視為**待處置的舊名**。
...
> ⚠️ 本標記涵蓋 **§0.0 全節**；⛔ **不**涵蓋 §0 的類型表與下方「變更級別」段——那兩者
> 現行有效。刻意逐字寫出涵蓋範圍，⛔ 不讓讀者自己猜一個 §0 裡「哪些算目標狀態」的界線。
