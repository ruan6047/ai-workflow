...
「實務曾三次停在 `📦已合併` 留下假活卡」；2026-08-24T20:55+08:00 實測**四張卡正停在該值**，
最久 **21 天**（cpbl#52，其餘為 #90 19 天、#81 10 天、#79 9 天）。
⇒ 它不是瞬間通過的點，是真實位置。
...
3. **不回填既有卡的簡介**（2026-08-24T20:55+08:00 實測 189 張）——回填本身要另開卡，且 ⛔ 未驗證簡介對 AI 判斷相關性的實效。
...
⚠️ 實得 **13 行對應 11 句**（表列 11 行）。兩處差異須逐一對上：第 4、5 行**同屬第 4 句**
...
終態卡以 `archiveProjectV2Item` 移出活卡視圖。2026-08-24T20:55+08:00 實測 Project 4 共
189 個 item 中 127 個是終態，佔 **67%**。

⚠️ **本節的數字一律附到分鐘的時戳，⛔ 不是只寫日期。** 理由是實測的：同一日 20:40 量到
188／127／68%，20:55 再量已是 189／127／67%——⭐ **看板是即時快照，把它的數字寫進永久
文件必然過期**。附時戳使該句從「關於現在的宣稱」變成「關於某一刻的事實」，⇒ 它永遠為真。
本卡稍早正因只寫日期而在 canonical 留下三處過期值（跨家族查核 R3-001）。⛔ **維護卡不封存**——那會重演「交付後沒人看」。
