...
**鐵律**：
1. **一卡一 worktree**，目錄名**必須含卡 ID**（`<repo>-<卡ID>`）——這是後續對帳的唯一依據。
2. **worktree 屬於卡，不屬於分支**。同一張卡（含其修復卡，§5.1）**全程共用一個 worktree**；退回時**保留**（要回去修），不另開新的。
3. **清理者＝merge 者**（查核通過後執行 merge 的人／PM）。**merge 是一組四個動作，缺一不可**，不得只做第 1 步：
   ```bash
   git switch main && git merge --no-ff ai/<模型>/<卡ID>   # 1. 合併（帶 §6.1 trailers）
   git push origin main
   git branch -d ai/<模型>/<卡ID>                          # 2. 刪本地
   git push origin --delete ai/<模型>/<卡ID>               # 3. 刪遠端
   git worktree remove ../<repo>-<卡ID>                    # 4. 移除 worktree
   #    → 隨即更新卡片狀態（📦已合併／🏁完成）並依 §6.4 封存
   ```
   > `git worktree remove` **前先 `cd` 離開該目錄**，否則 shell 的 cwd 消失，後續指令會噴 `fatal: Unable to read current working directory` 並靜默失敗。
4. **對帳（PM 定期執行）**：`git worktree list` 的每個目錄，都必須對得上 Ledger 裡一張**活卡**（`🔨執行中`／`🔍待查核`／`↩退回`）。
   - **有 worktree、卡已結案** → 收尾沒做完（漏了第 2–4 步）。
   - **有活卡、無 worktree** → 執行者提前刪了，查核者無處進駐（違反 §3.1 審核交接）。
   - 兩者皆為**流程缺陷，須立即補正並記入卡片 Log**。
5. 卡片進入 `🏁完成` 或封存時，**該卡的 worktree 與分支必須皆為 0**。這是結案的出口條件之一（承 §2 鐵律 4）。
...
2. **卡 ID＝`<原卡ID>-FIX<n>`**，並沿用**原卡的 worktree**（§3.1.1 鐵律 2）。
...
- worktree：<../repo-卡ID>（結案時必須已移除—§3.1.1）
...
```
> **worktree 欄是 §3.1.1 對帳的依據**：`git worktree list` 的每個目錄都要對得上一張活卡的這一欄。缺欄＝對不了帳＝殘留會長期無人發現。
