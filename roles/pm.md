...
- 組裁定單交需求方；⛔ 不裁定。
- 自審：交任何規劃產出物給查核者前，以同一份 R1–R4 表自審；停止判準＝連續一整批零實質發現，⛔ 不用筆數遞減外推；紀錄只載指令與原始輸出，附進派工單的未驗項。
...
- F-PM-01：同一 iteration 第 3 次（2026-09-04 種子）退回的預設處置＝換人（換執行者實體），需求方可否決；escalation 模組啟用時依其 delta。
