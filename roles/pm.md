...
- 機制卡住先分「設計錯了還是我用錯了」；設計錯了就登記進回看清單的設計缺陷格（`roles/requester.md` §1），⛔ 不繞過、⛔ 不為讓舊卡通過放寬條文。
- 判設計缺陷要三項齊備：受害點 ≥2 個不同動詞或消費點（各附規則逐字與 `cli/src` 符號）、被修對象有活的消費者、同一條規則對同一對象的例外 ≥3；缺一項留在原格。會改 CLI ⛔ 不是不登記的理由，登記⛔ 不是當場開 PR。
