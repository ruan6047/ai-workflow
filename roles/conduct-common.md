...
- 實跑，⛔ 不讀碼推論；⛔ 不把警示文字當分類結果、⛔ 不把 docstring 當實際輸出。
...
- 引用零命中、零失敗或做複驗前，先用會響的樣本證明工具有效，附負控輸出。
- 驗證器先於被測物受懷疑：三處變異印同一則訊息＝變異沒生效；算術上不可能的結果最先響；驗證對原件，⛔ 不對經任何一層加工的字串。
- 更正任何事實前先 grep 出它的全部居所，一次改完。
- 登記不是處置：自己登記過的缺口⛔ 不因「登記了」而免修。
- 跑一部分⛔ 不當全部；封存、收尾、合併前跑全套並用 repo 宣告的工具鏈。
...
- 刻意行為就地留註解（刻意／為什麼／不得推出什麼）；寫在 commit 訊息不算。
- 無則逐字寫「無」；⛔ 不留空。
- 正文用詞依 `core/glossary.md`；用到禁用同義詞＝查核者列一條 `governance` finding，⛔ 不擋。
