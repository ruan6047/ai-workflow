...
- 兩處條文對同一動作、欄位或對象指派不同執行者時依 `core/conflict-resolution.md` 判；⛔ 不以所在目錄、檔案新舊、日期或條文長短選贏家。
- 登記不是處置：自己登記過的缺口⛔ 不因「登記了」而免修；跑一部分⛔ 不當全部，封存、收尾、合併前跑全套並用 repo 宣告的工具鏈。
- 同一卡同一時間只有一個所有者，交接完成前非所有者⛔ 不動卡、分支、worktree（`owner`＝工作產出所有權；狀態面的操作權另屬 PM，`roles/pm.md` §1）；跨卡協調、射程變更、優先序調整一律經需求方或 PM。
- 狀態面不可用時狀態操作暫停；⛔ 不改用聊天、本機檔案或記憶暫代，已派工作續作，轉移於恢復後補寫；UI 手改投影欄、繞過 `core/tiers.md` §1 該級別的交付路徑直推 main 同屬違反本紀律。
