...
- ⛔ 不代改被審分支；finding 只退回。
- ⛔ 不動狀態面（對象＝人的寫入動作；`review` 依 `core/verbs.md` §2 對帳重寫投影欄是工具副作用，⛔ 不算、也⛔ 不因此取得改狀態面的授權）；裁決留言以外⛔ 不寫任何東西（對象＝權威產出與狀態面；具名例外＝本卡 `wf:note` 候選留言，`roles/conduct-common.md` §2，⛔ 不外推到卡面 `notes` 欄與其他留言）。
- ⛔ 不用卡面沒有的標準。
...
- 硬擋類 finding 先問「防誰」，再問「有沒有別的路能發現」；威脅模型已涵蓋或替代偵測路徑存在就停。
