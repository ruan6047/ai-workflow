...
claim_event_id: <有效 lease 的 claim_event_id>
...
立此節的成因是一個已發生的實例，不是預防性潔癖：當時的卡面範本（`WF-REDESIGN-W2B` 移除，理由見 [`template-migration-map.md`](template-migration-map.md)）路由行那一行早就逐字寫出了分隔符，缺的從來不是文件詳盡度——**沒有任何一處宣告它們是保留字，而寫入端對值不做檢查**，於是 `open` 寫得出一個 `assign` 讀不回的值（`ai-workflow#21` 的往返缺陷，由 `#37` 修復）。文件把格式寫得再清楚，也擋不住一個不檢查的寫入端。
...
決議紀錄 §六逐字列出**五份交接文件**——派工包／交付報告／派審詞／裁決／狀態變更裁定單
——並要求它們共用一個信封。本節是那個信封的權威定義；`templates/` 下各份範本**帶四段
標題、⛔ 不各自重寫定義**。結案報告（`closeout-report.md`）雖不在那五份之列，同罩本節：
它與派審詞同屬 PM 自產出物，`stage-rules/pm-conduct.md` 四之末逐字要求兩者的信封段落
與未驗三分類編號化。

**四段標題逐字**（⛔ 不得改寫字面——各範本的存在性判準即是 grep 這四行）：
...
⚠️ 上列四行在本節刻意寫成**行內碼**而非真標題：本檔的每個契約符號在對帳表上的錨點取
「命中行往上最近的標題」，插進四個真標題會把其後所有符號的錨點指到信封名去。⛔ 不得
由此推出「範本裡也該寫成行內碼」——範本裡它們是**真標題**，那正是 grep 判存在性的對象。

| 段 | 承載什麼 | 為什麼分開 |
|---|---|---|
| 一 · 卡與身分 | 卡ID／Issue、級別、Initiative 與 spec 基線、階段與輪次、from／to 角色，以及**模型行** | 收件者第一件事是判「這份東西是哪張卡的哪一輪、誰交給誰」。基線與輪次寫錯，後面每一格都對不上 |
| 二 · 身分自述 | 撰寫者的 session ID ＋該則訊息定位 | `author` 恆為同一個 GitHub 帳號 ⇒ 這是唯一可核對的身分訊號（`stage-rules/list-intake-requirements.md` 條件 5）。它與段一的角色欄不同：段一寫「誰負責」，段二寫「拿什麼去核對」 |
| 三 · 機械指令 | 收件者要跑的指令逐條，含 rc 取法與預期輸出面 | 完整性宣稱一律由指令輸出產生。指令不寫進交接文件，下一手就只能照自己的慣例跑——⛔ 而慣例換一個實體就會掉 |
| 四 · 已知未驗項 | **逐項＋各自原因**，三分類：`驗不了`／`沒去驗`／`刻意不驗`（後者含委給下一手的理由） | 承接 canonical §6.4.2。裸列未驗項讀起來像「都驗過了、只是列一下」；分類逼出「這一項到底有沒有人會去驗」 |

**模型行**（段一內，⛔ 不另立一段）——`stage-rules/pm-conduct.md` 一逐字「信封必含
『實際模型／卡面建議／偏離理由』一行」：
...
**四段是下界，⛔ 不是全部**：各範本在四段之外另有自己的內容段（派工包的標準條款、交付
報告的失誤登記、裁決的 findings…），本節不管轄那些。本節管轄的是「換一個實體來寫，這
四件事不會掉」。

⚠️ **本節⛔ 沒有機械執行者。** 沒有任何東西會在有人寫出缺段的交接文件時轉紅；四段標題
是文字約定，其執行面是 `stage-rules/` 各階段檔的 ④——PM／需求方對「該有的段落有沒有」逐條看。
依 §3.2 末段的自我適用規則，指不出執行者所在的檔與行 ⇒ 本節寫成**約定**，⛔ 不得寫成
強制。⛔ 也不得由此推出「四段可省」：約定被違反時的偵測靠人，⛔ 不是沒有偵測。
