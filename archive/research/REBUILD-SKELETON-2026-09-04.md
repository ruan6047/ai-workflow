...
密度不均是上一版的病（01 空洞、03 空洞）。每種檔固定節次、固定上限（上限＝天花板，不是配額）。理由與來歷的形狀＝釘在該條規則後的 `→ archive/…` 連結（決策 9；2026-09-07 需求方裁甲：⛔ 不放檔尾的檔級連結——整份舊卡動輒千行、讀者分不出哪條規則來自哪段，且本輪 23 份跨實體裁決當依據用 0 次；已刪 19 檔 20 行，`core/tiers.md` 指向舊規則特定節者保留）；萃取稿 `docs/research/extract/` 是填規則的輸入，填完後整目錄移入 `archive/research/`，規則正文⛔ 不引用它。
...
| 任一非終態（記法 `<非終態>`；核心四值＋模組加的狀態，2026-09-07 補記） | 阻塞 | 寫 `blocked.from`；`--ruling` 種類＝`wf-ruling` kind=block，缺留言或缺鍵皆印 |
| 阻塞 | from（必為非終態） | 解除 |
...
- 目錄樹與節次是設計，未經任何試填；行數上限是估計（core 120 於 2026-09-07 依量測改 150，見決策紀錄）。
