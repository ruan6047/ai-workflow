...
（2026-08-29/30 原話）「明明是為了流程順暢 現在卻卡了一個月」「現在是不是又無限開卡了」「目前框架內ＣＬＩ有點過量尤其是很多不該由ＣＬＩ處理的都變成由ＣＬＩ處裡」

**量化佐證**（皆 2026-08-29/30 量測，屬歷史快照；⛔ 敘述不承載現況數——量法與重量指令歸各 spec artifact）：開卡 5.2 張/日 vs 推進下界 ≥3.9 次/日（日差分，⛔ 非嚴格不等式）；在動的卡恆 0–5 張；審核 APPROVE 105 : REQUEST_CHANGES 150（⚠️ 退回率⛔ 不得單獨當 KPI——高 RC 可能是查核在做事，須配輪次與 severity 分佈讀）；⭐ 一天四張同族卡（開卡產生器——承重的質性事實）；CLI 26 天 17,194 行、fix 47 > feat 38、refactor 4。（階段欄 88.6% 空已自佐證移除——該欄刻意休眠，屬設計⛔ 非缺陷。）

**N=30 樣本量依據（2026-08-30 推導、08-31 隨指標②改基重述）**：對任何落在 0.3–0.5 區間的比例型指標（含指標②各終態占比），n=30 的 SE≈8–9pp，足辨有感位移而 n=10 噪音±16pp 蓋過效果；歷史節奏下 30 張常態卡≈2–4 週，兼避 cutover 初期混亂。⛔ 原推導基於退回率基線，該指標已因兩制輪次語意不同棄用——本句即重述後的依據。
**三指標之預登基線（供對照；皆歷史快照）**：①開卡 5.2 張/日（2026-08-29，舊制「開卡即卡」折算）；②66 張完成常態卡中「曾退回重做」（卡級 iteration≥1，regime-neutral 旗標）佔 42%、終態比完成 124：停止 34（2026-08-30 看板）；③CLI 17,194 行／144 拒收點（2026-08-30）。

**回顧三指標的 regime-neutral 定義（新舊制語意相同，供 N=30 對照）**：①清單項→卡轉化（舊制以「開卡即卡」折算）；②卡級終態結果分佈（完成／停止／曾退回重做——⛔ 不用輪次計數，兩制輪次語意不同）；③CLI 行數與拒收點數（diff 產生）。
